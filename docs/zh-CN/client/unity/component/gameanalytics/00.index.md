# 安装指南

本文档详细介绍如何将 GameFrameX GameAnalytics 游戏数据分析组件安装到您的 Unity 项目中。

## 概述

GameFrameX GameAnalytics 组件是 Unity 游戏框架的核心模块之一，用于集成游戏数据分析功能。该组件支持多种数据分析平台的接入，提供统一的事件上报接口和计时功能，方便开发者在不同数据分析平台之间切换或同时使用多个平台。

**前置要求：**
- Unity 2017.1 或更高版本
- 已安装 GameFrameX Runtime 核心库

## 安装方式

GameFrameX GameAnalytics 组件支持三种安装方式，您可以根据项目需求选择最适合的方法。

### 方式一：通过 manifest.json 安装

在您的 Unity 项目中，找到 `Packages` 文件夹下的 `manifest.json` 文件，在 `dependencies` 节点下添加以下内容：

```json
{
  "dependencies": {
    "com.gameframex.unity.gameanalytics": "https://github.com/AlianBlank/com.gameframex.unity.gameanalytics.git"
  }
}
```

> Source: [README.md](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/README.md#L150-L156)

修改完成后，Unity 会自动下载并安装该包。

### 方式二：通过 Unity Package Manager 安装

1. 打开 Unity 编辑器
2. 选择菜单 `Window` → `Package Manager`
3. 点击左上角的 `+` 按钮
4. 选择 `Add package from git URL...`
5. 输入以下 Git URL：
   ```
   https://github.com/AlianBlank/com.gameframex.unity.gameanalytics.git
   ```
6. 点击 `Add` 按钮等待安装完成

> Source: [README.md](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/README.md#L156)

### 方式三：直接下载安装

1. 从 GitHub 仓库下载最新的 Release 版本或克隆整个仓库
2. 将下载的文件夹复制到 Unity 项目的 `Packages` 目录下
3. Unity 会自动识别并加载该包

## 安装验证

安装完成后，您可以通过以下方式验证是否安装成功：

### 检查 Package Manager

在 Unity Package Manager 窗口中，确认 `Game Frame X Game Analytics` 已出现在包列表中，并且显示正确的版本号。

```json
{
  "name": "com.gameframex.unity.gameanalytics",
  "version": "1.2.0"
}
```

> Source: [package.json](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/package.json#L1-L6)

### 检查程序集定义

确认以下程序集定义文件已正确加载：

```mermaid
flowchart TD
    subgraph Runtime["Runtime 模块"]
        ASM1[GameFrameX.GameAnalytics.Runtime.asmdef]
        ASM1 -->|"references"| CORE[GameFrameX.Runtime]
    end
    
    subgraph Editor["Editor 模块"]
        ASM2[GameFrameX.GameAnalytics.Editor.asmdef]
    end
    
    CORE -.->|"依赖"| ASM1
```

> Source: [GameFrameX.GameAnalytics.Runtime.asmdef](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/Runtime/GameFrameX.GameAnalytics.Runtime.asmdef#L3-L5)

## 组件配置

安装完成后，需要在 Unity 场景中添加 GameAnalytics 组件进行配置。

### 添加组件

1. 在 Unity 编辑器中，创建或打开一个场景
2. 在 Hierarchy 面板中，选择一个 GameObject（或创建新对象）
3. 在 Inspector 面板中，点击 `Add Component` 按钮
4. 搜索并选择 `GameAnalytics` 组件
5. 组件将自动添加到选定的 GameObject 上

> Source: [GameAnalyticsComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/Runtime/GameAnalytics/GameAnalyticsComponent.cs#L13-L14)

### 配置分析提供者

GameAnalytics 组件支持配置多个数据分析提供者。在 Inspector 面板中：

```csharp
[DisallowMultipleComponent]
[AddComponentMenu("Game Framework/GameAnalytics")]
public sealed class GameAnalyticsComponent : GameFrameworkComponent
```

> Source: [GameAnalyticsComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/Runtime/GameAnalytics/GameAnalyticsComponent.cs#L12-L14)

1. 找到 **游戏数据分析组件列表** 属性
2. 点击 `+` 按钮添加新的提供者
3. 从下拉菜单中选择要使用的数据分析平台类型
4. 根据所选平台填写相应的配置参数

```mermaid
flowchart TD
    subgraph Config["组件配置流程"]
        A[添加 GameAnalytics 组件] --> B[配置提供者列表]
        B --> C[选择平台类型]
        C --> D[填写平台参数]
        D --> E[保存场景]
    end
```

## 依赖项说明

GameFrameX GameAnalytics 组件依赖于以下模块：

| 依赖项 | 说明 | 必需 |
|--------|------|------|
| GameFrameX.Runtime | 核心运行时库 | 是 |

> Source: [GameFrameX.GameAnalytics.Runtime.asmdef](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/Runtime/GameFrameX.GameAnalytics.Runtime.asmdef)

**重要提示：** 确保您已安装 GameFrameX Runtime 核心库，否则组件将无法正常工作。

## 快速验证

安装和配置完成后，可以通过以下代码验证组件是否正常工作：

```csharp
using GameFrameX.GameAnalytics.Runtime;

// 获取组件实例
var gameAnalytics = UnityEngine.GameObject.FindObjectOfType<GameAnalyticsComponent>();

// 检查是否初始化成功
if (gameAnalytics != null)
{
    // 上报测试事件
    gameAnalytics.Event("test_event");
    UnityEngine.Debug.Log("GameAnalytics 组件安装成功！");
}
```

> Source: [GameAnalyticsComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.gameanalytics/blob/main/Runtime/GameAnalytics/GameAnalyticsComponent.cs#L246-L265)

## 常见问题

### 安装后编译报错

如果遇到编译错误，请检查：
1. 是否已正确安装 GameFrameX Runtime
2. Unity 版本是否符合要求（2017.1+）
3. manifest.json 语法是否正确

### 组件无法添加到场景

确保：
1. 已正确导入包
2. 在 Runtime 文件夹下有正确的程序集定义
3. Unity 项目已刷新（有时需要重启编辑器）

### Inspector 面板配置不显示

如果 Inspector 中看不到 GameAnalytics 组件的配置面板，可能需要：
1. 重新导入包
2. 检查 Editor 程序集是否正确加载
3. 确认 Unity Editor 版本兼容性

## 相关链接

- [插件介绍](../1-overview/index) - 了解更多关于 GameAnalytics 组件的功能特性
- [快速开始](../3-getting-started/index) - 开始使用 GameAnalytics 进行数据上报
- [核心架构](../4-core-concepts/index) - 了解组件的内部架构设计
- [编辑器配置](../6-configuration/index) - 详细了解编辑器配置选项
- [故障排除](../7-troubleshooting/index) - 常见问题与解决方案
