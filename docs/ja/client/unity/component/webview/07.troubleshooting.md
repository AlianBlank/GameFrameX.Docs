# 常见问题

本文档收集了使用 GameFrameX WebView コンポーネント时可能遇到的常见问题及其解決策。

## Overview

GameFrameX WebView コンポーネントは Unity WebView 封装库，基づく [gree/unity-webview](https://github.com/gree/unity-webview) 実装。该コンポーネント采用インターフェース设计模式，を通じて `IWebViewManager` インターフェース提供跨平台的 WebView 機能。

> **注意**：本包仅提供 WebView 的インターフェース定义和 Unity コンポーネント，具体的平台実装（如 Android/iOS WebView）必要额外安装对应的実装包。

## よくある質問与解決策

### 问题 1：WebView 不显示

#### 症状
添加 WebViewComponent 到シーン后，呼び出し `Show(url)` メソッド但 WebView 没有显示。

#### 可能原因
1. 平台実装包未安装
2. WebViewManager 未正确初期化
3. URL 格式不正确

#### 解決策
```csharp
// 确保正确取得 WebViewComponent インスタンス
var webView = FindObjectOfType<WebViewComponent>();
if (webView != null)
{
    // 使用有效的 HTTPS URL
    webView.Show("https://gameframex.doc.alianblank.com");
}
else
{
    Debug.LogError("WebViewComponent 未在シーン中找到！");
}
```

> Source: [WebViewComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.webview/blob/main/Runtime/WebViewComponent.cs#L36-L49)

---

### 问题 2：Fatal: Web View manager is invalid

#### 症状
运行时控制台输出 `Log.Fatal("Web View manager is invalid.")` 错误。

#### 可能原因
1. **コア依赖包未安装**：本コンポーネント依赖 `com.gameframex.unity` 包
2. **平台実装包缺失**：没有安装对应平台的 WebView 実装包
3. **模块注册失败**：WebViewManager 模块未正确注册到フレームワーク

#### 解決策

1. **安装コア依赖包**：
   ```
   を通じて Unity Package Manager 添加：https://github.com/gameframex/com.gameframex.unity.git
   ```

2. **安装平台実装包**：に基づいて目标平台安装对应的 WebView 実装包（如果存在）

3. **检查模块注册**：确保在ゲーム启动时正确初期化了 GameFrameX フレームワーク

---

### 问题 3：iOS/Android 平台 WebView 空白

#### 症状
在 iOS 或 Android 平台上 WebView 显示为空白，页面无法加载。

#### 可能原因
1. 网络权限未配置
2. HTTPS 证书问题
3. 平台特定配置缺失

#### 解決策

**对于 iOS**：
- 在 `Info.plist` 中添加网络权限：
```xml
<key>NSAppTransportSecurity</key>
<dict>
    <key>NSAllowsArbitraryLoads</key>
    <true/>
</dict>
```

**对于 Android**：
- 确保在 `AndroidManifest.xml` 中添加了 Internet 权限：
```xml
<uses-permission android:name="android.permission.INTERNET" />
```

---

### 问题 4：JavaScript 交互无响应

#### 症状
呼び出し `ExecuteJavaScript()` メソッド后，JavaScript 代码没有执行或没有效果。

#### 可能原因
1. WebView 未完全加载
2. JavaScript 代码语法错误
3. 呼び出し时机过早

#### 解決策
```csharp
// 建议在 WebView 加载完成后再执行 JavaScript
// 可能を通じて订阅イベント来监听加载完成ステータス
var webView = FindObjectOfType<WebViewComponent>();
webView.Show("https://example.com");

// 延迟执行 JavaScript，确保页面已加载
Invoke(nameof(ExecuteCustomJavaScript), 2.0f);

void ExecuteCustomJavaScript()
{
    webView.ExecuteJavaScript("console.log('Hello from Unity!');");
}
```

> Source: [WebViewComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.webview/blob/main/Runtime/WebViewComponent.cs#L59-L66)

---

### 问题 5：WebView 全屏显示異常

#### 症状
呼び出し `MakeFullScreen()` メソッド后，WebView 显示位置或大小不正确。

#### 解決策
```csharp
var webView = FindObjectOfType<WebViewComponent>();
webView.Show("https://example.com");
// 等待页面加载完成后，再設定为全屏
Invoke(nameof(MakeFullScreen), 1.0f);

void MakeFullScreen()
{
    webView.MakeFullScreen();
}
```

> Source: [WebViewComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.webview/blob/main/Runtime/WebViewComponent.cs#L54-L57)

---

### 问题 6：多个 WebView インスタンス冲突

#### 症状
シーン中有多个 WebViewComponent 时，出现显示混乱或イベント错乱。

#### 解決策
- 建议シーン中只保留一个 WebViewComponent インスタンス
- 使用单例模式管理 WebView：
```csharp
public class WebViewManager : MonoBehaviour
{
    private static WebViewManager _instance;
    public static WebViewManager Instance => _instance;
    
    private WebViewComponent _webView;
    
    private void Awake()
    {
        _instance = this;
        _webView = GetComponent<WebViewComponent>();
    }
    
    public void ShowWebView(string url)
    {
        _webView.Show(url);
    }
}
```

---

### 问题 7：Unity 编辑器中无法测试

#### 症状
在 Unity 编辑器模式下运行，WebView 不显示或报错。

#### 原因説明
WebView 機能必要在真机或对应平台的模拟器上测试，Unity 编辑器本身不サポート原生 WebView 显示。

#### 解決策
- 将プロジェクト构建到目标平台（iOS/Android）进行测试
- 使用平台関連的模拟器

---

## 诊断手順

### 基础诊断フロー

```mermaid
flowchart TD
    Start([开始诊断]) --> Step1{控制台有错误?}
    Step1 -->|是| Step2{错误信息为"Web View manager is invalid"?}
    Step1 -->|否| Step3{检查代码呼び出し}
    
    Step2 -->|是| Step4[检查コア依赖包]
    Step2 -->|否| Step5[に基づいて具体错误排查]
    
    Step4 --> Step6{依赖包已安装?}
    Step6 -->|否| Step7[安装 com.gameframex.unity]
    Step6 -->|是| Step8[检查平台実装包]
    
    Step7 --> Step9[重新构建プロジェクト]
    Step8 --> Step9
    Step9 --> End1([结束])
    
    Step3 --> Step10{URL 有效?}
    Step10 -->|否| Step11[使用有效 HTTPS URL]
    Step10 -->|是| Step12[检查平台权限配置]
    
    Step5 --> End1
    Step11 --> End1
    Step12 --> End1
```

### 调试检查清单

| 检查项 | 操作メソッド |
|--------|----------|
| コア包依赖 | 检查 Package Manager 中是否安装 `com.gameframex.unity` |
| シーン配置 | 确认 GameObject 上正确添加了 WebViewComponent |
| 代码呼び出し | 确认在 Start() 或之后呼び出し Show() メソッド |
| URL 有效性 | 确认使用有效的 HTTPS URL |
| 平台权限 | 检查各平台的权限配置ファイル |
| イベント监听 | 添加イベント监听查看是否有错误イベント触发 |

---

## 架构説明

### コンポーネント关系

```mermaid
graph TD
    subgraph Unity["Unity シーン"]
        GameObj[GameObject]
        WVC[WebViewComponent]
    end
    
    subgraph GameFrameX["GameFrameX フレームワーク"]
        GFEntry[GameFrameworkEntry]
        GFModule[GameFrameworkModule]
    end
    
    subgraph Interface["インターフェース层"]
        IWM[IWebViewManager]
        BWM[BaseWebViewManager]
    end
    
    subgraph Platform["平台実装"]
        AndroidWM[Android WebView Manager]
        iOSWM[iOS WebView Manager]
    end
    
    GameObj --> WVC
    WVC --> GFEntry
    GFEntry --> GFModule
    GFModule --> IWM
    IWM --> BWM
    BWM --> AndroidWM
    BWM --> iOSWM
```

### 工作フロー

```mermaid
sequenceDiagram
    participant U as Unity シーン
    participant WVC as WebViewComponent
    participant GFE as GameFrameworkEntry
    participant WMM as WebViewManager (平台実装)
    participant Native as 原生 WebView

    U->>WVC: 添加コンポーネント到 GameObject
    WVC->>GFE: GetModule<IWebViewManager>()
    GFE-->>WVC: 返回 WebViewManager インスタンス
    WVC->>WMM: Initialize()
    U->>WVC: Show(url)
    WVC->>WMM: Show(url)
    WMM->>Native: 作成并显示原生 WebView
    Native-->>WMM: WebView インスタンス
    WMM-->>WVC: 操作完成
```

---

## 関連リンク

- [概述](./1-overview) - WebView コンポーネント概述与機能介绍
- [安装](./2-installation) - 详细安装指南
- [快速入门](./3-getting-started) - 快速开始使用 WebView
- [API 参考](./4-api-reference) - 完整的 API メソッド説明
- [平台配置](./5-platforms) - 各平台特定配置説明
- [イベント系统](./6-events) - WebView イベント系统详解
- [GameFrameX コアフレームワーク](https://github.com/GameFrameX/com.gameframex.unity) - コアフレームワーク仓库
- [gree/unity-webview](https://github.com/gree/unity-webview) - 原始 WebView 插件
