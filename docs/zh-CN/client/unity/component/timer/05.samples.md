...
本章节提供 Timer 计时器组件的完整使用示例，展示如何在 Unity 项目中实际应用各种定时任务功能。

## 概述

Timer 组件提供了一套完整的定时任务解决方案，支持以下几种定时模式：
- **循环定时任务**：按固定间隔重复执行
- **单次定时任务**：延迟指定时间后执行一次
- **帧更新任务**：每帧执行一次，适用于需要持续监控的场景

这些示例基于实际的源代码编写，可以直接在你的项目中使用。

## 基础使用示例

### 添加循环定时任务

以下示例展示如何创建一个每间隔 1 秒执行一次，总共执行 5 次的定时任务：
...
以下是一个更完整的示例，展示如何在游戏开发中实际应用 Timer 组件：
...
TimerManager 提供了全局异常捕获功能，可以防止定时任务中的异常导致游戏崩溃：
...
| 每帧检测 | AddUpdate | 适用于输入检测、状态监控 |
| 倒计时功能 | Add + AddOnce | 循环显示倒计时，结束时触发结束回调 |
| 技能冷却 | Add | 周期性检查冷却时间 |
