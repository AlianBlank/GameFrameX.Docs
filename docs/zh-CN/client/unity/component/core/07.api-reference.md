...
本文档提供 GameFrameX 框架所有公共 API 的完整参考，包括核心类、接口、方法签名和使用说明。

## 概述

GameFrameX 框架采用模块化架构设计，核心 API 主要分为以下几个部分：

- **框架入口**：`GameFrameworkEntry` 和 `GameEntry` 提供框架初始化和组件管理
- **基础组件**：`BaseComponent` 提供游戏运行时基础设置
- **引用池**：`ReferencePool` 提供高性能内存管理
- **事件系统**：`EventPool<T>` 提供类型安全的事件机制
- **任务池**：`TaskPool<T>` 提供异步任务管理
- **对象池**：`ObjectPoolComponent` 提供游戏对象复用
- **工具类**：`Utility` 命名空间提供各类辅助功能
...
游戏基础组件，提供帧率控制、游戏速度管理、后台运行设置等核心功能。
...
| `GameSpeed` | `float` | 获取或设置游戏速度（0 表示暂停） |
| `IsGamePaused` | `bool` | 获取游戏是否暂停 |
...
| 属性 | 类型 | 描述 |
|------|------|------|
| `EnableStrictCheck` | `bool` | 获取或设置是否开启强制类型检查 |
| `Count` | `int` | 获取引用池的数量 |
...
| 属性 | 类型 | 描述 |
|------|------|------|
| `EventHandlerCount` | `int` | 获取事件处理函数的数量 |
| `EventCount` | `int` | 获取队列中待处理事件的数量 |
...
- [运行时模块 - 事件系统](./5-runtime.event-system)
- [运行时模块 - 对象池系统](./5-runtime.object-pool)
- [运行时模块 - 引用池系统](./5-runtime.reference-pool)
- [运行时模块 - 任务池系统](./5-runtime.task-pool)
- [运行时模块 - 工具类](./5-runtime.utility-classes)
