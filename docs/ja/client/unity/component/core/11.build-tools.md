...
GameFrameX 提供します一套完整的 Unity 编辑器ビルドツール，サポート多平台构建、ホットアップデート（HybridCLR）集成、构建钩子扩展等機能。

## 概要

ビルドツール模块位于 `Editor/` ディレクトリ下，主要包含以下コアコンポーネント：

- **BuildProductHelper** - 主构建助手，提供多平台构建機能
- **BuildHotfixHelper** - ホットアップデート代码管理助手
- **Hook 机制** - 构建前后的扩展钩子インターフェース

这些工具を通じて Unity Editor 菜单 `GameFrameX/Build` 提供统一的构建エントリーポイント，简化了跨平台构建フロー。

## 架构

```mermaid
flowchart TD
    subgraph BuildTools["ビルドツール模块"]
        BH[BuildProductHelper<br/>主构建助手]
        BPH[BuildPostProcessHelper<br/>构建后处理]
        
        subgraph Hooks["构建钩子"]
            IPH[IBuilderPreHookHandler<br/>构建前钩子インターフェース]
            IPO[IBuilderPostHookHandler<br/>构建后钩子インターフェース]
        end
        
        subgraph Hotfix["ホットアップデートサポート"]
            BHH[BuildHotfixHelper<br/>ホットアップデート助手]
...
| コンポーネント | ファイル路径 | 説明 |
|------|----------|------|
| BuildProductHelper | `Editor/BuildProduct/BuildProductHelper.cs` | 主构建助手，サポート Windows、Mac、Android、iOS、WebGL 平台构建 |
| BuildPostProcessHelper | `Editor/BuildProduct/BuildPostProcessHelper.cs` | 构建后自动更新版本号 |
| IBuilderPreHookHandler | `Editor/BuildProduct/IBuilderPreHookHandler.cs` | 构建前钩子インターフェース |
| IBuilderPostHookHandler | `Editor/BuildProduct/IBuilderPostHookHandler.cs` | 构建后钩子インターフェース |
| BuildHotfixHelper | `Editor/BuildHotfix/BuildHotfixHelper.cs` | ホットアップデート代码复制和管理 |
| HotFixEditorCompilerHelper | `Editor/BuildHotfix/HotFixEditorCompilerHelper.cs` | 编辑器编译控制 |
| HotFixAssemblyDefinitionHelper | `Editor/BuildHotfix/HotFixAssemblyDefinitionHelper.cs` | 程序集定义ファイル修改 |
| BuildWebGLToolsWithHybridCLR | `Editor/BuildWebGLTools/BuildWebGLToolsWithHybridCLR.cs` | WebGL HybridCLR サポート |

## 菜单项

ビルドツールを通じて Unity Editor 菜单提供统一的构建エントリーポイント，位于 `GameFrameX/Build` 菜单下：

| 菜单项 | 機能 |
|--------|------|
| Windows X64 | 构建 Windows x64 可执行ファイル |
| Windows X32 | 构建 Windows x32 可执行ファイル |
...
| Copy HotFix Code | 复制ホットアップデート代码到 Bundles/Code ディレクトリ |
| Copy AOT Code | 复制 AOT 代码到 Bundles/AOTCode ディレクトリ |
| HotFix Editor Compiler Add | 添加ホットアップデート程序集的编辑器编译指令 |
| HotFix Editor Compiler Remove | 移除ホットアップデート程序集的编辑器编译指令 |

## コア機能

### 多平台构建

BuildProductHelper サポート以下平台的构建：
...
// Android Studio プロジェクト导出（Debug/Release）
...
を通じて実装 `IBuilderPreHookHandler` 和 `IBuilderPostHookHandler` インターフェース，可能在构建前后执行自定义逻辑：
...
```csharp
// 复制ホットアップデート DLL 到 Assets/Bundles/Code
[MenuItem("GameFrameX/Build/Copy HotFix Code(复制ホットアップデート代码到Assets>Bundles>Code)", false, 10)]
...
在构建过程中，必要临时控制ホットアップデート程序集的编译范围：

```csharp
// 移除ホットアップデート程序集的编辑器编译指令
[MenuItem("GameFrameX/Build/HotFix Editor Compiler Remove(移除ホットアップデート程序集的编辑器编译指令)", false, 15)]
...
// 增加ホットアップデート程序集的编辑器编译指令
[MenuItem("GameFrameX/Build/HotFix Editor Compiler Add(增加ホットアップデート程序集的编辑器编译指令)", false, 16)]
...
1. **选择目标平台**：在 Unity Editor 中打开シーン
2. **設定构建目标**：を通じて `File > Build Settings` 选择目标平台
3. **执行构建**：を通じて `GameFrameX/Build` 菜单选择对应的构建选项
4. **查看输出**：构建完成后，在 `Builds/{平台}/` ディレクトリ下查看输出

### 使用构建钩子扩展

作成一个自定义构建钩子，に使用在构建前清理旧リソース：
...
- [小ゲーム平台适配](./6-editor-tools.mini-game-platform) - 了解如何适配微信、字节跳动等小ゲーム平台
- [パッケージ管理工具](./6-editor-tools.package-management) - 了解パッケージ管理機能
- [运行时模块概述](./5-runtime.base) - 了解运行时基础コンポーネント
- [イベント系统](./5-runtime.event-system) - 了解イベント系统
- [オブジェクトプールシステム](./5-runtime.object-pool) - 了解オブジェクトプールシステム
