...
本文档介绍 GameFrameX Download 插件的基础配置方法，包括组件添加、参数设置和初始化流程。

## 概述

Download 组件是 GameFrameX 框架中的下载管理模块，负责处理所有下载相关任务。在使用下载功能之前，需要正确配置 `DownloadComponent`。该组件采用多代理并发下载机制，支持断点续传、优先级调度和实时进度更新。

## 添加 DownloadComponent

### 通过 Unity 编辑器添加

在 Unity 场景中创建一个空游戏对象，然后添加 `Download` 组件：

1. 在 Unity 编辑器中，右键点击 Hierarchy 面板
2. 选择 **GameObject** → **Create Empty** 创建一个空游戏对象
...
在 Unity 编辑器的 Inspector 面板中，DownloadComponent 包含以下可配置属性：
...
| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| Instance Root | Transform | null | 下载代理实例的父级 Transform，为空时自动创建 |
| Download Agent Helper Type | string | UnityWebRequestDownloadAgentHelper | 下载代理辅助器的类型名称 |
| Custom Download Agent Helper | DownloadAgentHelperBase | null | 自定义下载代理辅助器实例 |
...
指定下载代理 GameObject 的父级 Transform。如果不设置，系统会自动创建一个名为 "Download Agent Instances" 的空对象作为父节点。
...
#### 2. Download Agent Helper Type（下载代理辅助器类型）

指定用于执行实际下载操作辅助器的类型。插件内置了以下实现：

| 类型名称 | 说明 |
|----------|------|
| UnityWebRequestDownloadAgentHelper | 使用 Unity 的 UnityWebRequest 进行下载（推荐） |
| WWWDownloadAgentHelper | 使用过时的 WWW 类进行下载（兼容旧版本） |

#### 3. Download Agent Helper Count（下载代理数量）

设置并发下载代理的数量。每个代理一次只能处理一个下载任务，但可以同时运行多个代理以实现并行下载。
...
**建议配置：**
- 小型资源（< 10MB）：2-3 个代理
- 中型资源（10-100MB）：3-5 个代理
- 大型资源（> 100MB）：5-8 个代理
- 最大支持：16 个代理

#### 4. Timeout（超时时间）

设置单个下载任务的最大等待时间（秒）。超过此时间后，下载将被视为失败并触发错误事件。
...
设置将下载缓冲区写入磁盘的临界大小。该参数仅在启用断点续传功能时有效，用于控制数据刷新频率。
...
### 默认实现：UnityWebRequestDownloadAgentHelper

插件默认使用 `UnityWebRequestDownloadAgentHelper` 作为下载代理辅助器，它基于 Unity 的 `UnityWebRequest` API 实现，支持：
...
要创建自定义下载代理，需要继承 `DownloadAgentHelperBase` 并实现相关接口：
...
DownloadComponent 提供了以下公开属性供代码访问：
...
| FlushSize | int | 获取或设置缓冲区写入磁盘的临界大小 |
...
DownloadComponent 依赖于以下组件和服务：

1. **EventComponent** - 必须组件，用于触发下载事件通知
2. **GameFrameworkEntry** - 框架入口，提供模块初始化
3. **GameFrameworkModule** - 基础模块基类
