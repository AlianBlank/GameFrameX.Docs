# 基础数据配置

基础数据配置功能允许开发者设置全局通用的请求参数，这些参数会自动附加到所有HTTP请求中，无需在每次请求时重复设置。

## 概述

在使用 WebComponent 进行HTTP请求时，很多场景下需要为所有请求添加共同的参数，例如：

- **身份验证**：API Token、用户会话ID
- **应用信息**：应用版本、设备平台、渠道标识
- **通用参数**：语言设置、地区编码

基础数据配置正是为解决这一需求而设计。通过预先配置基础数据，开发者可以一次性设置这些通用参数，后续的所有请求都会自动包含这些数据，大大简化了代码并减少了重复工作。

### 设计原理

基础数据采用「合并而非替换」的策略：

1. **基础数据**在初始化时设置一次，贯穿整个应用生命周期
2. **请求数据**在每次发起请求时单独指定
3. **合并时**：如果请求数据中包含与基础数据相同的键，请求数据会**覆盖**基础数据
4. 这种设计既保证了基础数据的便利性，又保留了个别请求的灵活性

## 架构

基础数据配置是 WebManager 的核心功能之一，与请求处理流程紧密集成。

```mermaid
flowchart TD
    subgraph Client["客户端调用层"]
        WC[WebComponent]
    end

    subgraph Manager["管理器层"]
        WM[WebManager]
    end

    subgraph DataStore["数据存储层"]
        BF[m_BaseForm<br/>基础表单数据]
        BH[m_BaseHeader<br/>基础请求头]
        BQ[m_BaseQueryString<br/>基础查询参数]
    end

    subgraph Merger["数据合并层"]
        MF[MergeForm]
        MH[MergeHeader]
        MQ[MergeQueryString]
    end

    subgraph Request["请求执行层"]
        HTTP[HttpWebRequest]
    end

    WC -->|调用| WM
    WM -->|管理| BF
    WM -->|管理| BH
    WM -->|管理| BQ
    MF -->|合并表单| BF
    MH -->|合并请求头| BH
    MQ -->|合并查询参数| BQ
    MF --> HTTP
    MH --> HTTP
    MQ --> HTTP
```

### 各组件职责

| 组件 | 职责 |
|------|------|
| WebComponent | 提供Unity组件接口，代理IWebManager的调用 |
| WebManager | 实现基础数据管理和请求合并逻辑 |
| m_BaseForm | 存储全局表单数据字典 |
| m_BaseHeader | 存储全局请求头字典 |
| m_BaseQueryString | 存储全局查询参数字典 |
| Merge* 方法 | 执行基础数据与请求数据的合并 |

## 核心功能

WebManager 提供了三类基础数据的管理功能：

### 1. 基础表单数据（Base Form）

基础表单数据会被添加到POST请求的请求体中，适用于需要随请求发送的通用业务参数。

```csharp
// 添加基础表单数据
webComponent.AddBaseForm("app_version", "1.0.0");
webComponent.AddBaseForm("platform", "iOS");
webComponent.AddBaseForm("device_id", "ABC123");
```

> Source: [WebManager.cs](https://github.com/GameFrameX/com.gameframex.unity.web/blob/main/Runtime/Web/WebManager.cs#L591-L594)

### 2. 基础请求头数据（Base Header）

基础请求头数据会被添加到所有HTTP请求的请求头中，常用于身份验证和内容协商。

```csharp
// 添加基础请求头数据
webComponent.AddBaseHeader("Authorization", "Bearer token123");
webComponent.AddBaseHeader("Accept", "application/json");
webComponent.AddBaseHeader("X-Request-ID", Guid.NewGuid().ToString());
```

> Source: [WebManager.cs](https://github.com/GameFrameX/com.gameframex.unity.web/blob/main/Runtime/Web/WebManager.cs#L621-L624)

### 3. 基础查询参数数据（Base Query String）

基础查询参数会被附加到请求URL后面，适用于API版本、语言设置等常见参数。

```csharp
// 添加基础查询参数数据
webComponent.AddBaseQueryString("v", "2");
webComponent.AddBaseQueryString("lang", "zh-CN");
webComponent.AddBaseQueryString("timestamp", DateTimeOffset.Now.ToUnixTimeSeconds().ToString());
```

> Source: [WebManager.cs](https://github.com/GameFrameX/com.gameframex.unity.web/blob/main/Runtime/Web/WebManager.cs#L651-L654)

### 数据管理方法

每类基础数据都提供三种管理方法：

| 方法 | 功能 | 说明 |
|------|------|------|
| Add* | 添加数据 | 键存在时覆盖，不存在时新增 |
| Remove* | 移除数据 | 指定的键不存在时静默忽略 |
| Clear* | 清空数据 | 一次性清除该类型所有基础数据 |

```csharp
// 移除单个基础数据
webComponent.RemoveBaseForm("device_id");

// 清空所有基础表单数据
webComponent.ClearBaseForm();

// 清空所有基础请求头
webComponent.ClearBaseHeader();

// 清空所有基础查询参数
webComponent.ClearBaseQueryString();
```

> Source: [IWebManager.cs](https://github.com/GameFrameX/com.gameframex.unity.web/blob/main/Runtime/Web/IWebManager.cs#L147-L198)

## 数据合并流程

当发起HTTP请求时，基础数据会与请求参数自动合并。理解合并机制有助于更好地使用该功能。

```mermaid
sequenceDiagram
    participant Client as 调用方
    participant WM as WebManager
    participant Merge as Merge方法
    participant Req as HttpWebRequest

    Client->>WM: PostToString(url, form)
    WM->>Merge: MergeForm(form)
    
    Note over Merge: 1. 复制基础数据<br/>2. 合并请求数据（覆盖）
    Merge-->>WM: 合并后的表单
    
    WM->>Req: 设置请求体
    Req-->>Client: 请求发送
    
    Note over Client,Req: 请求头和查询参数<br/>采用相同合并流程
```

### 合并优先级

```mermaid
flowchart LR
    subgraph Input["输入数据"]
        A[基础数据<br/>BaseData]
        B[请求数据<br/>RequestData]
    end

    subgraph MergeProcess["合并处理"]
        C{相同键?}
    end

    subgraph Output["最终数据"]
        D[请求数据<br/>覆盖基础数据]
    end

    A --> C
    B --> C
    C -->|是| D
    C -->|否| D
```

**合并规则**：
- 如果请求数据中包含某个键，其值会**覆盖**基础数据中相同键的值
- 如果请求数据中不包含某个键，则使用基础数据中的值
- 基础数据本身不会被修改

### 合并实现示例

```csharp
/// <summary>
/// 合并表单数据
/// </summary>
private Dictionary<string, object> MergeForm(Dictionary<string, object> form)
{
    if (m_BaseForm.Count == 0)
    {
        return form;
    }

    // 复制基础数据
    var result = new Dictionary<string, object>(m_BaseForm);

    // 覆盖/添加外部数据
    if (form != null)
    {
        foreach (var kv in form)
        {
            result[kv.Key] = kv.Value;
        }
    }

    return result;
}
```

> Source: [WebManager.cs](https://github.com/GameFrameX/com.gameframex.unity.web/blob/main/Runtime/Web/WebManager.cs#L685-L705)

## 使用场景示例

### 场景一：应用启动时初始化基础数据

在游戏或应用启动时配置全局参数：

```csharp
public class GameLauncher : MonoBehaviour
{
    private WebComponent _webComponent;

    private void Start()
    {
        _webComponent = GetComponent<WebComponent>();
        
        // 配置基础查询参数
        _webComponent.AddBaseQueryString("v", "2.0.0");
        _webComponent.AddBaseQueryString("platform", Application.platform.ToString());
        
        // 配置基础请求头
        _webComponent.AddBaseHeader("Authorization", $"Bearer {GetAuthToken()}");
        
        // 配置基础表单数据
        _webComponent.AddBaseForm("app_id", GetAppId());
    }
    
    private string GetAuthToken() { /* 获取Token */ return "token"; }
    private string GetAppId() { /* 获取AppID */ return "app123"; }
}
```

### 场景二：API请求时自动携带基础数据

配置完成后，后续请求无需再手动添加这些参数：

```csharp
// 后续请求会自动携带之前设置的基础数据
// 最终请求: POST /api/user/info?v=2.0.0&platform=iOS 
//           Header: Authorization: Bearer token123
//           Body: {app_id: "app123", name: "test"}
var result = await _webComponent.PostToString(
    "https://api.example.com/api/user/info",
    new Dictionary<string, object> { { "name", "test" } }
);
```

### 场景三：临时覆盖基础数据

在某些请求中需要使用不同值时，直接在请求中指定即可：

```csharp
// 即使基础数据中设置了 v=2.0.0，此请求仍使用 v=3.0.0
var result = await _webComponent.PostToString(
    "https://api.example.com/api/v3/feature",
    new Dictionary<string, object> { { "name", "test" } },
    new Dictionary<string, string> { { "v", "3.0.0" } } // 覆盖基础查询参数
);
```

### 场景四：用户登录后更新Token

用户登录成功后更新认证信息：

```csharp
public async Task OnUserLogin(string newToken)
{
    // 移除旧Token
    _webComponent.RemoveBaseHeader("Authorization");
    
    // 添加新Token
    _webComponent.AddBaseHeader("Authorization", $"Bearer {newToken}");
}
```

### 场景五：切换API环境

在开发/生产环境间切换时：

```csharp
public void SwitchEnvironment(bool isProduction)
{
    string baseUrl = isProduction 
        ? "https://api.production.com" 
        : "https://api.dev.com";
    
    // 清空并重新设置基础查询参数
    _webComponent.ClearBaseQueryString();
    _webComponent.AddBaseQueryString("env", isProduction ? "prod" : "dev");
    _webComponent.AddBaseQueryString("debug", isProduction ? "false" : "true");
}
```

## API 参考

### IWebManager 接口方法

以下方法定义在 `IWebManager` 接口中，由 `WebManager` 类实现：

#### 表单数据管理

| 方法 | 描述 |
|------|------|
| `AddBaseForm(string key, object value)` | 添加基础表单数据 |
| `RemoveBaseForm(string key)` | 移除指定的基础表单数据 |
| `ClearBaseForm()` | 清空所有基础表单数据 |

#### 请求头管理

| 方法 | 描述 |
|------|------|
| `AddBaseHeader(string key, string value)` | 添加基础请求头数据 |
| `RemoveBaseHeader(string key)` | 移除指定的基础请求头数据 |
| `ClearBaseHeader()` | 清空所有基础请求头数据 |

#### 查询参数管理

| 方法 | 描述 |
|------|------|
| `AddBaseQueryString(string key, string value)` | 添加基础查询参数数据 |
| `RemoveBaseQueryString(string key)` | 移除指定的基础查询参数数据 |
| `ClearBaseQueryString()` | 清空所有基础查询参数数据 |

> 完整的 IWebManager 接口定义请参见 [API 参考 - IWebManager 接口](./04.iwebmanager-interface)。

### WebComponent 代理方法

`WebComponent` 继承自 `GameFrameworkComponent`，内部持有 `IWebManager` 实例，并代理所有基础数据管理方法的调用。使用方式与直接调用 `IWebManager` 完全相同。

> WebComponent 的完整说明请参见 [API 参考 - WebComponent 组件](./08.web-component)。

## 相关链接

- [超时配置](./10.timeout-settings) - 配置请求超时时间
- [IWebManager 接口](./04.iwebmanager-interface) - Web管理器的完整接口定义
- [WebComponent 组件](./08.web-component) - Unity组件使用说明
- [请求结果类型](./06.result-types) - 了解请求返回的结果类型
