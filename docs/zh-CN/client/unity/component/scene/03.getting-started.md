...
本指南将帮助您快速上手 GameFrameX Scene 场景管理组件。通过本文档，您将了解如何在自己的 Unity 项目中配置和使用场景管理系统。

## 概述

GameFrameX Scene 组件是 GameFrameX 游戏框架的核心模块之一，提供统一的场景管理功能。该组件基于 YooAsset 资源管理系统构建，支持异步场景加载、卸载、状态追踪以及场景事件回调。核心功能由 `SceneComponent` 统一封装对外提供服务，通过事件系统实现场景状态变化的响应。
...
上图展示了 Scene 组件的层次结构。用户通过 `SceneComponent` 组件进行场景操作，该组件内部调用 `GameSceneManager` 进行实际的管理工作，而 `GameSceneManager` 则依赖 `IAssetManager` 与 YooAsset 资源系统进行交互。`EventComponent` 负责触发各类场景事件，供游戏逻辑响应。

## 前置条件

在使用 Scene 组件之前，请确保项目满足以下条件：

1. 已安装 Unity 2020.3 或更高版本
2. 项目中已正确配置 YooAsset 资源管理系统
3. 已将 GameFrameX 框架其他依赖模块添加到项目中（EventComponent、Asset 模块等）

## 基本使用流程

### 步骤一：添加 Scene 组件

在 Unity 场景中创建一个空物体或使用现有的 GameEntry 物体，然后添加 `SceneComponent` 组件。该组件会自动注册到 GameFrameX 框架中。
...
组件属性说明：
- `Enable LoadScene Update Event`：启用场景加载进度更新事件
- `Enable LoadScene Dependency Asset Event`：启用场景依赖资源加载事件

### 步骤二：获取组件实例

在游戏代码中可以通过 `GameEntry` 获取 SceneComponent 实例：
...
加载新场景时会先卸载所有已加载的场景，适用于游戏主菜单、关卡切换等场景。

**累加模式 (LoadSceneMode.Additive)**
在现有场景基础上叠加加载新场景，适用于开放世界、动态关卡加载等场景。
...
SceneComponent 支持设置场景的显示顺序，确保主摄像机正确指向当前激活的场景：
...
上图展示了场景加载的完整流程。从用户调用 LoadScene 开始，经过参数验证、资源加载、状态管理，最终触发相应事件通知用户代码。
...
1. **场景路径格式**：场景资源名称必须以 `Assets/` 开头，以 `.unity` 结尾，例如：`Assets/Scenes/MainMenu.unity`

2. **状态检查**：在加载新场景之前，建议先检查场景是否已在加载中或已加载，避免重复加载导致异常

3. **资源依赖**：确保 YooAsset 已正确配置场景的 AssetBundle 打包规则

4. **事件订阅时机**：建议在 Start 或更早的生命周期中订阅事件，在 OnDestroy 中取消订阅，避免内存泄漏

5. **异步操作**：LoadScene 是异步方法，需要使用 await 等待加载完成

## 相关链接

- [核心功能](./4-core-features) - 了解更多场景管理的高级功能
- [API 参考](./5-api-reference) - 查看完整的 API 文档
- [事件系统](./6-events) - 了解场景事件系统的详细信息
- [常见问题](./7-troubleshooting) - 排查常见问题
- [安装指南](./2-installation) - 查看安装配置说明
