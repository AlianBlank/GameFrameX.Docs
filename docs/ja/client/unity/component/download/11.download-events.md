...
このドキュメント詳細介绍 GameFrameX 下载モジュール中的イベント系统，含みます下载任务イベント和下载代理イベント两大类。

## 概要

GameFrameX 下载モジュール采用イベント驱动アーキテクチャ，通过統一された的イベント系统実装下载ステータス的实时通知。イベント系统分为两个层次：

1. **下载任务イベント** - 针对整个下载任务的ステータス变更通知
2. **下载代理イベント** - 针对下载代理辅助器的底层ステータス通知

这种分层設計使得開発者〜できます根据需求选择不同粒度的イベント监听，既〜できます监听高层下载任务ステータス，也〜できます监听底层代理数据传输ステータス。

## イベントアーキテクチャ

下载イベント系统采用リリース-購読モード，通过 `EventComponent` 統一された管理和分发イベント。
...
1. 客户端通过 `EventComponent` 購読感兴趣的下载イベント
2. 当呼び出し `DownloadComponent.AddDownload()` 追加下载任务时，任务进入下载队列
3. `DownloadManager` 调度下载代理执行实际下载操作
4. 下载过程中产生的各类イベント通过 `DownloadComponent` 转发到 `EventComponent`
5. 購読者收到イベント通知并执行相应逻辑

## 下载任务イベント

下载任务イベント在 `IDownloadManager` インターフェース中定義，由 `DownloadComponent` 統一された暴露给外部使用。这些イベント提供します下载任务完全なライフサイクル中的关键ステータス变更通知。
...
| イベント名称 | イベントパラメータ类 | トリガー时机 |
|---------|-----------|---------|
| 下载开始 | `DownloadStartEventArgs` | 下载任务开始执行时 |
| 下载アップデート | `DownloadUpdateEventArgs` | 下载進捗アップデート时 |
| 下载成功 | `DownloadSuccessEventArgs` | 下载成功完成时 |
| 下载失敗 | `DownloadFailureEventArgs` | 下载失敗时 |

### DownloadStartEventArgs

下载开始イベント，在下载任务被下载代理受信并开始処理时トリガー。
...
下载アップデートイベント，在下载过程中持续トリガー，用于报告当前下载進捗。
...
下载代理イベント定義在 `DownloadAgentHelperBase` 抽象类中，提供更底层的下载ステータス通知。这些イベント主な被 `DownloadManager` 内部使用，用于追踪下载代理的工作ステータス。
...
| イベント名称 | イベントパラメータ类 | トリガー时机 |
|---------|-----------|---------|
| 代理アップデート数据流 | `DownloadAgentHelperUpdateBytesEventArgs` | 下载代理受信到新的数据块时 |
| 代理アップデート数据大小 | `DownloadAgentHelperUpdateLengthEventArgs` | 下载代理下载数据大小变化时 |
| 代理完成 | `DownloadAgentHelperCompleteEventArgs` | 下载代理完成下载时 |
| 代理エラー | `DownloadAgentHelperErrorEventArgs` | 下载代理发生エラー时 |

### DownloadAgentHelperUpdateBytesEventArgs

代理アップデート数据流イベント，提供下载数据的原始字节内容。
...
開発者〜できます通过 `EventComponent` 購読下载イベント，监听下载ステータス的变化。
...
| `ErrorMessage` | `string` | エラー説明情報 | Failure |
| `UserData` | `object` | ユーザー自定義数据 | 全部 |
...
| `DeleteDownloading` | `bool` | 是否〜する必要があります削除〜中下载的ファイル | Error |
| `ErrorMessage` | `string` | エラー説明情報 | Error |

## イベント使用お勧め

1. **选择适当的イベント粒度**
   - 监听下载任务イベント（`DownloadStart`/`DownloadUpdate`/`DownloadSuccess`/`DownloadFailure`）用于业务逻辑処理
   - 监听代理イベント用于実装下载進捗条或デバッグ

2. **イベントパラメータ池化**
   - 所有イベントパラメータ通过 `ReferencePool` 进行池化管理
   - 使用 `Create()` メソッド作成イベント，通过 `Clear()` メソッド回收

3. **エラー処理**
   - 始终購読 `DownloadFailure` イベント以捕获下载失敗情况
   - 失敗イベント中的 `ErrorMessage` パッケージ含詳細的エラー情報

## 関連リンク

- [代理イベント](./6-events.agent-events) - 下载代理底层イベント详解
- [DownloadComponent](./4-core-concepts.download-component) - 下载コンポーネント详解
- [下载代理](./4-core-concepts.download-agent) - 下载代理コンセプト説明
- [API リファレンス](./5-api-reference) - 完全な的 API リファレンスドキュメント
