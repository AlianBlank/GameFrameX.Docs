...
UIForm은 GameFrameX 프레임워크에서 모든 UI 창체의 기본 클래스로, 완전한 생명주기 관리, 가시성 제어, 이벤트 구독 및 UI 그룹 통합을 제공합니다.

## 개요

UIForm은 UI 시스템의 핵심 클래스로, Unity의 `MonoBehaviour`를 상속하고 `IUIForm` 인터페이스를 구현합니다. 창체에 완전한 생명주기 관리를 제공하여 초기화, 표시, 숨김, 닫기, 일시 정지, 복원 등의 상태 전환을 처리합니다.
