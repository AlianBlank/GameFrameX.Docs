# 对象池系统

对象池系统是 GameFrameX フレームワーク的コアモジュール之一，用于管理游戏对象的复用，避免频繁的对象作成和破棄带来的性能开销和 GC 压力。该系统提供します完全な的对象ライフサイクル管理，含みます对象的取得、回收、自动解放等機能。

## 概要

在游戏开发中，频繁作成和破棄对象（如子弹、特效、UI 元素等）是常见的性能瓶颈。对象池系统通过预先作成一定数量的对象并重复利用，显著降低了内存分配开销和 GC トリガー频率。

GameFrameX 对象池系统提供以下コア機能：

- **タイプ安全的ジェネリック池管理**：サポート强タイプ的对象池操作
- **单次/多次取得モード**：柔軟控制对象的复用策略
- **自动解放机制**：基于时间过期和容量阈值的自动回收
- **优先级系统**：精细控制对象解放优先级
- **锁定机制**：保护重要な对象不被意外解放

## アーキテクチャ

```mermaid
classDiagram
    class ObjectPoolComponent {
        +IObjectPoolManager ObjectPoolManager
        +int Count
        +HasObjectPool~T~() bool
        +GetObjectPool~T~() IObjectPool~T~
        +CreateSingleSpawnObjectPool~T~() IObjectPool~T~
        +CreateMultiSpawnObjectPool~T~() IObjectPool~T~
        +DestroyObjectPool~T~() bool
    }
    
    class ObjectPoolManager {
        -Dictionary~TypeNamePair, ObjectPoolBase~ m_ObjectPools
        +int Count
        +CreateSingleSpawnObjectPool~T~() IObjectPool~T~
        +CreateMultiSpawnObjectPool~T~() IObjectPool~T~
        +Release()
        +ReleaseAllUnused()
    }
    
    class ObjectPoolBase {
        <<abstract>>
        +string Name
        +string FullName
        +Type ObjectType
        +int Count
        +bool AllowMultiSpawn
        +float AutoReleaseInterval
        +int Capacity
        +float ExpireTime
        +int Priority
    }
    
    class IObjectPool~T~ {
        <<interface>>
        +Register(T, bool)
        +CanSpawn() bool
        +Spawn() T
        +Unspawn(T)
        +Release()
    }
    
    class ObjectPool~T~ {
        -GameFrameworkMultiDictionary~string, Object~T~~ m_Objects
        -Dictionary~object, Object~T~~ m_ObjectMap
        -bool m_AllowMultiSpawn
    }
    
    class ObjectBase {
        <<abstract>>
        +string Name
        +object Target
        +bool Locked
        +int Priority
        +bool CustomCanReleaseFlag
        +DateTime LastUseTime
        +OnSpawn()
        +OnUnspawn()
        +Release(bool)
    }
    
    class Object~T~ {
        <<internal>>
        +T m_Object
        +int m_SpawnCount
        +Spawn() T
        +Unspawn()
    }
    
    ObjectPoolComponent --> ObjectPoolManager : uses
    ObjectPoolManager --> ObjectPoolBase : manages
    ObjectPoolBase <|.. ObjectPool~T~ : implements
    IObjectPool~T~ <|.. ObjectPool~T~ : implements
    ObjectPool~T~ --> Object~T~ : wraps
    ObjectBase <|-- CustomObject : user implements
    Object~T~ --> ObjectBase : wraps
```

### コンポーネント説明

| コンポーネント | 説明 | 层级 |
|------|------|------|
| `ObjectPoolComponent` | Unity コンポーネント，挂在 GameObject 上供シーン使用 | API 层 |
| `ObjectPoolManager` | コアマネージャー，管理所有对象池 | コア层 |
| `ObjectPoolBase` | 对象池抽象基底クラス | 抽象層 |
| `ObjectPool<T>` | ジェネリック对象池実装 | 実装層 |
| `ObjectBase` | 可池化对象的抽象基底クラス | 抽象層 |
| `Object<T>` | 内部对象パッケージ装器 | 内部実装 |

## コアコンセプト

### 对象池タイプ

系统サポート两种对象池タイプ：

**1. 单次取得对象池 (Single Spawn)**
- 同一时间只能有一个消费者使用对象
- 適しています独占性リソース，如プレイヤー角色、载具等

```csharp
// 创建一个单次获取对象池
IObjectPool<Bullet> bulletPool = objectPoolComponent.CreateSingleSpawnObjectPool<Bullet>("Bullet Pool", 100, 60f, 10);
```

> Source: [Runtime/ObjectPool/ObjectPoolComponent.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolComponent.cs#L257-L260)

**2. 多次取得对象池 (Multi Spawn)**
- 同一时间〜できます同時に被多个消费者使用
- 適しています可复用リソース，如子弹、特效等

```csharp
// 创建一个多次获取对象池
IObjectPool<Effect> effectPool = objectPoolComponent.CreateMultiSpawnObjectPool<Effect>("Effect Pool", 50, 30f, 5);
```

> Source: [Runtime/ObjectPool/ObjectPoolComponent.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolComponent.cs#L655-L658)

### 关键パラメータ

| パラメータ | 説明 | デフォルト值 |
|------|------|--------|
| `name` | 对象池名称，用于标识和查找 | 空字符串 |
| `capacity` | 对象池容量，最大对象数量 | int.MaxValue |
| `expireTime` | 对象过期时间（秒），超过此时间未使用则可被解放 | float.MaxValue（永不过期） |
| `autoReleaseInterval` | 自动解放间隔（秒），每隔此时间尝试解放过期对象 | 依赖作成パラメータ |
| `priority` | 对象池优先级，影响解放顺序 | 0 |

## 使用フロー

```mermaid
sequenceDiagram
    participant Client as 客户端代码
    participant Component as ObjectPoolComponent
    participant Manager as ObjectPoolManager
    participant Pool as ObjectPool<T>
    participant Object as ObjectBase

    Note over Client: 1. 创建对象池
    Client->>Component: CreateSingleSpawnObjectPool<T>()
    Component->>Manager: CreateSingleSpawnObjectPool<T>()
    Manager->>Pool: new ObjectPool<T>()
    Manager-->>Component: 返回 IObjectPool<T>
    
    Note over Client: 2. 注册对象到池
    Client->>Component: Register(obj, spawned)
    Component->>Pool: Register(obj, spawned)
    Pool->>Object: 创建并添加到池
    
    Note over Client: 3. 获取对象
    Client->>Component: Spawn()
    Component->>Pool: Spawn()
    Pool->>Object: 设置 SpawnCount++
    Pool-->>Client: 返回 T
    
    Note over Client: 4. 回收对象
    Client->>Component: Unspawn(obj)
    Component->>Pool: Unspawn(obj)
    Pool->>Object: 设置 SpawnCount--
    
    Note over Client: 5. 释放对象（自动/手动）
    Manager->>Pool: Release()
    Pool->>Object: 调用 Release() 回调
```

## 作成可池化对象

要使用对象池系统，〜する必要があります継承 `ObjectBase` 类并実装相关メソッド：

```csharp
using GameFrameX.ObjectPool;
using GameFrameX.Runtime;

public class Bullet : ObjectBase
{
    private BulletController m_Controller;
    
    // 初始化对象
    public void Initialize(BulletController controller)
    {
        Initialize(null, controller.Target, 0);
        m_Controller = controller;
    }
    
    // 对象被获取时调用
    protected internal override void OnSpawn()
    {
        base.OnSpawn();
        m_Controller.gameObject.SetActive(true);
    }
    
    // 对象被回收时调用
    protected internal override void OnUnspawn()
    {
        base.OnUnspawn();
        m_Controller.gameObject.SetActive(false);
    }
    
    // 对象被释放时调用
    protected internal override void Release(bool isShutdown)
    {
        if (isShutdown || m_Controller == null)
        {
            if (m_Controller != null)
            {
                UnityEngine.Object.Destroy(m_Controller.gameObject);
            }
        }
        m_Controller = null;
    }
}
```

> Source: [Runtime/ObjectPool/ObjectBase.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectBase.cs#L201-L223)

### 拡張メソッド

系统还提供します拡張メソッド方便ユーザー使用：

```csharp
// 扩展方法示例（伪代码）
public static class ObjectPoolExtensions
{
    public static T Spawn<T>(this IObjectPool<T> pool) where T : ObjectBase
    {
        return pool.Spawn();
    }
    
    public static void Unspawn<T>(this IObjectPool<T> pool, T obj) where T : ObjectBase
    {
        pool.Unspawn(obj);
    }
}
```

## 使用例

### 基本用法

```csharp
using GameFrameX.Runtime;
using GameFrameX.ObjectPool;
using UnityEngine;

// 1. 获取对象池组件
var objectPoolComponent = UnityEngine.Object.FindObjectOfType<ObjectPoolComponent>();

// 2. 创建对象池（多次获取模式，适合子弹）
var bulletPool = objectPoolComponent.CreateMultiSpawnObjectPool<Bullet>("Bullet Pool", 100, 60f);

// 3. 注册对象到池中
var bulletObj = new Bullet();
bulletObj.Initialize(bulletController);
bulletPool.Register(bulletObj, false);

// 4. 从池中获取对象
var bullet = bulletPool.Spawn();

// 5. 使用完毕后回收对象
bulletPool.Unspawn(bullet);
```

> Source: [Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs#L256-L292)

### 高级用法：自定義解放策略

```csharp
// 自定义释放过滤器
IObjectPool<Bullet> bulletPool = objectPoolComponent.GetObjectPool<Bullet>("Bullet Pool");

// 释放符合条件的对象
bulletPool.Release(10, (candidateObjects, toReleaseCount, expireTime) =>
{
    // 优先释放优先级低的对象
    var releaseList = new List<Bullet>();
    var sorted = candidateObjects.OrderByDescending(b => b.Priority)
                                 .ThenBy(b => b.LastUseTime);
    
    int count = 0;
    foreach (var bullet in sorted)
    {
        if (count >= toReleaseCount) break;
        releaseList.Add(bullet);
        count++;
    }
    return releaseList;
});
```

> Source: [Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs#L498-L529)

### 高级用法：对象锁定

```csharp
// 锁定对象，防止被自动释放
bulletPool.SetLocked(bullet, true);

// 解锁对象，允许被释放
bulletPool.SetLocked(bullet, false);
```

> Source: [Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs#L336-L374)

### 高级用法：設定优先级

```csharp
// 设置对象优先级（数值越大优先级越高，越晚被释放）
bulletPool.SetPriority(bullet, 100);
```

> Source: [Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolManager.ObjectPool.cs#L376-L414)

## 設定オプション

### ObjectPoolComponent 設定

在 Unity エディタ中，`ObjectPoolComponent` コンポーネント提供以下可設定项：

| 設定项 | タイプ | 説明 |
|--------|------|------|
| ImplementationComponentType | Type | 对象池マネージャー実装タイプ |
| InterfaceComponentType | Type | 对象池マネージャーインターフェースタイプ |

> 详情请リファレンス ObjectPoolComponent ソースコード

### 对象池パラメータ設定

| パラメータ | タイプ | デフォルト值 | 説明 |
|------|------|--------|------|
| name | string | "" | 对象池名称 |
| capacity | int | int.MaxValue | 对象池容量 |
| expireTime | float | float.MaxValue | 对象过期时间（秒） |
| autoReleaseInterval | float | float.MaxValue | 自动解放间隔 |
| priority | int | 0 | 对象池优先级 |

## API リファレンス

### ObjectPoolComponent

```csharp
// 创建单次获取对象池
IObjectPool<T> CreateSingleSpawnObjectPool<T>(string name, float autoReleaseInterval, int capacity, float expireTime, int priority)
```

**パラメータ：**
- `name` (string): 对象池名称
- `autoReleaseInterval` (float): 自动解放间隔秒数
- `capacity` (int): 对象池容量
- `expireTime` (float): 对象过期秒数
- `priority` (int): 对象池优先级

```csharp
// 创建多次获取对象池
IObjectPool<T> CreateMultiSpawnObjectPool<T>(string name, float autoReleaseInterval, int capacity, float expireTime, int priority)
```

> Source: [Runtime/ObjectPool/ObjectPoolComponent.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/ObjectPoolComponent.cs#L1026-L1029)

### IObjectPool<T>

```csharp
// 注册对象到池
void Register(T obj, bool spawned)

// 检查是否可以获取对象
bool CanSpawn(string name)

// 获取对象
T Spawn(string name)

// 回收对象
void Unspawn(T obj)

// 释放对象
void Release()
```

> Source: [Runtime/ObjectPool/IObjectPool.cs](https://github.com/GameFrameX/com.gameframex.unity/blob/main/Runtime/ObjectPool/IObjectPool.cs#L92-L129)

## 関連リンク

- [参照池系统](./5-runtime.reference-pool) - 另一种池化机制，用于管理参照タイプ对象
- [任务池系统](./5-runtime.task-pool) - 异步任务池管理
- [基本コンポーネント](./5-runtime.base) - GameFramework 基本コンポーネント
- [API リファレンス](./8-api-reference) - 完全な的 API ドキュメント
