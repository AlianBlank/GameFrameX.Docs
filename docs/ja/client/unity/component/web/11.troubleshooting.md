...
本文档汇总了 GameFrameX Unity Web 插件在使用过程中常见的 问题、原因分析以及对应的解決策。

## 概要

GameFrameX Web コンポーネントは高性能的 Unity HTTP 网络お願いします求库，サポート GET、POST お願いします求，可处理字符串、JSON、二进制数据等多种格式。在实际使用过程中，開発者经常会遇到一些问题，本文档将帮助你快速定位和解决这些问题。
...
**问题説明**：
在 WebGL 平台上发送网络お願いします求时，お願いします求没有响应或直接报错。

**可能原因**：
- WebGL 平台存在跨域限制
- 服务器未正确配置 CORS 头信息
- WebGL 不サポート多线程，所有お願いします求在主线程处理
...
2. **处理 OPTIONS 预检お願いします求**
   对于非简单お願いします求（带自定义头或 Content-Type 不是 application/x-www-form-urlencoded），服务器必須サポート OPTIONS 预检お願いします求。

3. **使用异步等待而非阻塞呼び出し**
   WebGL 不サポート多线程，建议使用 `await` 异步等待而不是 `.Wait()` 或 `.Result` 阻塞呼び出し：
...
1. **调整超时时间**
   ...
   ```csharp
   // メソッド一：を通じて WebComponent コンポーネント設定超时
   var webComponent = gameObject.GetOrAddComponent<WebComponent>();
   webComponent.Timeout = 60f; // 設定为 60 秒
   
   // メソッド二：を通じて IWebManager インターフェース設定超时
   var webManager = GameFrameworkEntry.GetModule<IWebManager>();
   webManager.Timeout = 60f; // 設定为 60 秒
   ```
...
**问题説明**：
在移动设备上发送 HTTPS お願いします求时，出现证书验证失败错误。
...
1. **使用默认证书绕过（仅に使用开发测试）**
   ...
   插件已内置 `DefaultBypassCertificate` クラスに使用绕过证书验证：
...
**问题説明**：
发送多个お願いします求后，部分お願いします求一直处于等待ステータス，长时间没有响应。
...
**问题説明**：
を通じて `AddBaseForm`、`AddBaseHeader` 等メソッド設定的基础数据，在实际お願いします求中没有被包含。

**可能原因**：
- 基础数据在お願いします求发送前未正确添加
- 添加后被清空
- 合并逻辑问题
...
**问题説明**：
同一个お願いします求在 Editor/PC 上正常工作，但在 WebGL 或移动平台上失败。
...
**问题説明**：
不清楚お願いします求失败时会抛出哪些異常クラス型。

**解決策**：

插件会に基づいて不同错误情况抛出以下異常：

| 異常クラス型 | 触发条件 | 处理建议 |
|---------|---------|---------|
| `TimeoutException` | お願いします求超时 | 增加超时时间或检查网络 |
| `WebException` | 网络错误 | 检查网络连接和服务器ステータス |
...
{
    Debug.LogError($"网络错误: {ex.Message}, ステータス: {ex.Status}");
}
...
```mermaid
flowchart TD
    A[网络お願いします求失败] --> B{是否有错误信息?}
    B -->|是| C{错误クラス型?}
    B -->|否| D[检查お願いします求是否发送成功]
