...
RPC（Remote Procedure Call，远程过程呼び出し）机制允许客户端向服务器发送请求并等待响应，是オンラインゲーム中最常用的客户端-服务器通信方法。

## 概要

GameFrameX 网络库提供します完全な的 RPC 実装，サポート：

- **同步呼び出し**：发送请求后阻塞等待响应
- **异步呼び出し**：使用 async/await モード异步等待响应
- **超时処理**：自动检测并処理 RPC 超时
- **エラー処理**：サポートエラー码コールバック和例外処理
- **イベント通知**：RPC 开始、完成、エラー的全程イベント监听

### メッセージタイプ

系统定義了三种メッセージタイプ用于不同的通信シーン：

| タイプ | インターフェース | 用途 |
|------|------|------|
| 请求メッセージ | `IRequestMessage` | 客户端发送给服务器的请求 |
| 响应メッセージ | `IResponseMessage` | 服务器返す给客户端的响应，パッケージ含 ErrorCode |
| 通知メッセージ | `INotifyMessage` | 服务器主动发送给客户端的通知，无需响应 |
...
在 GameFrameX 网络库中，所有网络メッセージ都継承自 `MessageObject` 基底クラス，并実装相应的インターフェース：
...
在 Unity エディタ中〜できます通过 NetworkComponent 設定 RPC 超时时间：
...
RpcState コンストラクタ要求超时时间不能小于 3000 毫秒：
...
**ジェネリックパラメータ：**
- `TResult` : 响应メッセージタイプ，〜しなければなりません継承 MessageObject 并実装 IResponseMessage

**パラメータ：**
- `messageObject` (MessageObject): 请求メッセージ对象
- `isIgnoreErrorCode` (bool): 是否忽略エラー码，デフォルト为 false

**返す：**
- `Task<TResult>`: 异步任务，パッケージ含响应メッセージ

**例外：**
- `ArgumentNullException`: messageObject 为 null
- `TimeoutException`: RPC 超时
- `GameFrameworkException`: 发送失敗
...
設定 RPC 开始发送时的コールバック函数。

**パラメータ：**
- `handler` (EventHandler<MessageObject>): コールバック処理函数
...
对于非 RPC タイプ的メッセージ（通知メッセージ），使用 `MessageHandlerAttribute` 来登録メッセージ処理器：
