...
DownloadComponent 是 Game Framework ダウンロード系统的 Unity コンポーネント层，作为 IDownloadManager インターフェース的外观（Facade），为 Unity プロジェクト提供便捷的ダウンロード機能集成。

## 概要

DownloadComponent 是ゲームフレームワークダウンロード模块的コア Unity コンポーネント，を継承 GameFrameworkComponent。它封装了 DownloadManager 的機能，并将其与 Unity 生命周期集成。作为一个 MonoBehaviour，開発者可能直接将其添加到ゲームシーン中，を通じて Inspector 面板或代码进行配置和使用。

该コンポーネント的コア设计目标是：
- 提供简洁的 Unity API インターフェース
- 管理多个ダウンロード代理（Download Agent）インスタンス
- を通じてイベント系统通知ダウンロード进度和ステータス变化
- サポート任务优先级、标签分クラス、断点续传等高级機能

## 架构

DownloadComponent 在整个ダウンロード系统中扮演着エントリーポイント层的角色，下图展示了其与各コンポーネント的关系：
...
flowchart TD
    ...
    subgraph AgentHelper["ダウンロード代理辅助器"]
...
**Unity 层**
- DownloadComponent：主コンポーネント，提供 Unity 友好的 API
- DownloadComponentInspector：Editor  Inspector，に使用可视化配置

**コア模块**
- DownloadManager：ダウンロード管理器実装，处理コア逻辑
- IDownloadManager：ダウンロード管理器インターフェース，定义规范

**任务池层**
- TaskPool：通用任务池，管理ダウンロード任务和代理
- DownloadAgent：ダウンロード代理，执行具体ダウンロード任务

**代理辅助器层**
- DownloadAgentHelperBase：ダウンロード代理辅助器基クラス
- UnityWebRequestDownloadAgentHelper：基づく UnityWebRequest 的実装
- WWWDownloadAgentHelper：基づく WWW 的実装（兼容旧版）
...
DownloadComponent 提供します丰富的ダウンロード任务管理機能：
...
DownloadComponent サポート异步ダウンロード模式，を通じて Task 返回ダウンロード结果：
...
コンポーネント提供します多个只读プロパティに使用监控ダウンロードステータス：
...
| プロパティ | クラス型 | 默认值 | 説明 |
|------|------|--------|------|
| InstanceRoot | Transform | null | ダウンロード代理インスタンス的根节点 |
| DownloadAgentHelperTypeName | string | UnityWebRequestDownloadAgentHelper | ダウンロード代理辅助器クラス型 |
| DownloadAgentHelperCount | int | 3 | ダウンロード代理インスタンス数量（1-16） |
| Timeout | float | 30f | ダウンロード超时时间（秒，0-120） |
...
DownloadComponent サポート断点续传機能，当ダウンロード中断后可能继续从上次位置ダウンロード：

```mermaid
flowchart TD
    Start[开始ダウンロード] --> Check{ファイル已存在?}
    
    Check -->|是| GetSize[取得ファイル大小]
    GetSize --> Resume[从断点开始ダウンロード]
    
    Check -->|否| Full[从头开始ダウンロード]
    ...
    Update --> Complete{ダウンロード完成?}
    
    Complete -->|否| DownloadLoop
    Complete -->|是| Success[ダウンロード成功]
    
    Error[ダウンロード错误] --> Retry{可重试?}
    Retry -->|是| Resume
    Retry -->|否| Fail[ダウンロード失败]
```

断点续传を通じて在 DownloadAgentHelper 中設定 HTTP Range 头実装：

```csharp
// UnityWebRequestDownloadAgentHelper.cs 中的断点续传実装
...
{
    Debug.Log($"ダウンロード开始: {e.DownloadUri}");
}

private void OnDownloadUpdate(object sender, DownloadUpdateEventArgs e)
{
    Debug.Log($"ダウンロード进度: {e.CurrentLength} bytes");
}
...
{
    Debug.Log($"ダウンロード失败: {e.ErrorMessage}");
}
```
...
public class ResourceUpdater
{
    ...
    private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
    {
        ...
        {
            Debug.Log($"リソースダウンロード完成: {info.Name}");
            CheckAllComplete();
        }
    }
}
...
DownloadComponent を通じて EventComponent 转发ダウンロードイベント，主要イベント包括：

| イベント | 説明 | EventArgs |
|------|------|-----------|
| DownloadStart | ダウンロード开始时触发 | DownloadStartEventArgs |
| DownloadUpdate | ダウンロード进度更新时触发 | DownloadUpdateEventArgs |
| DownloadSuccess | ダウンロード成功时触发 | DownloadSuccessEventArgs |
| DownloadFailure | ダウンロード失败时触发 | DownloadFailureEventArgs |
...
public sealed class DownloadSuccessEventArgs : GameEventArgs
{
    ...
    public string DownloadUri { get; }    // 原始ダウンロード地址
    public long CurrentLength { get; }     // 已ダウンロード大小
...
在 Unity 编辑器中，選択带有 DownloadComponent 的 GameObject，可能在 Inspector 面板中配置以下プロパティ：
...
DownloadComponent サポート自定义ダウンロード代理辅助器，默认使用 UnityWebRequestDownloadAgentHelper。開発者可能を通じて继承 DownloadAgentHelperBase 作成自定义実装：
...
DownloadComponent 使用内部クラス DownloadData 存储每个ダウンロード任务的元数据：

```csharp
sealed class DownloadData
{
    public TaskCompletionSource<bool> TCS { get; }  // に使用异步等待
    public object UserData { get; }                  // 用户数据
    public string Tag { get; }                        // 标签
    public int SerialId { get; }                      // 序列编号
    public string Url { get; }                        // ダウンロード地址
...
1. **Awake()**: 初期化 DownloadManager 模块，注册ダウンロードイベント
2. **Start()**: 取得 EventComponent，作成ダウンロード代理辅助器インスタンス
3. **Update()**: 由 GameFramework 自动呼び出し，处理任务池更新
4. **OnDestroy()**: 清理リソース（を通じて基クラス実装）
...
- [架构概述](./4-core-concepts.architecture) - ダウンロード系统整体架构
- [ダウンロード代理](./4-core-concepts.download-agent) - DownloadAgent 详解
- [API 参考 - プロパティ](./5-api-reference.properties) - 完整プロパティ列表
- [API 参考 - メソッド](./5-api-reference.methods) - 完整メソッド列表
- [ダウンロードイベント](./6-events.download-events) - ダウンロードイベント详解
- [编辑器配置](./7-configuration.editor-configuration) - Editor Inspector 配置
- [ダウンロード代理配置](./7-configuration.agent-configuration) - 代理辅助器配置
