# ProcedureManager

ProcedureManager 是 GameFrameX 流程Manages系统的核心Implements类，responsible forManages游戏中的各种流程状态及其切换。

## 概述

ProcedureManager Implements了 `IProcedureManager` 接口，Inherits自 `GameFrameworkModule` 基类，是流程Manages模块的主要Implements。它基于有限状态机（FSM）来Implements流程的Creates、切换和Manages功能。

**设计意图：**
- provides统一的流程Manages机制，supports游戏在不同阶段（如start、菜单、游戏中、pause、结算等）之间平滑切换
- 利用 FSM 的特性，ensure流程切换的可靠性和可追踪性
- 作为 GameFramework 的核心模块，supports热update和模块化扩展

## 架构

```mermaid
graph TD
    subgraph External["外部调用"]
        Client[客户端/Unity组件]
    end
    
    subgraph GameFramework["Game Framework 核心"]
        GFE["GameFrameworkEntry"]
        GFM["GameFrameworkModule (基类)"]
    end
    
    subgraph ProcedureModule["Procedure 模块"]
        PC["ProcedureComponent"]
        PM["ProcedureManager"]
        IPM["IProcedureManager 接口"]
        PB["ProcedureBase"]
    end
    
    subgraph FsmModule["FSM 模块 (依赖)"]
        FSM["IFsmManager"]
        FSMInst["IFsm&lt;IProcedureManager&gt;"]
    end
    
    Client -->|使用| PC
    PC -->|获取| IPM
    PM -->|实现| IPM
    PM -->|继承| GFM
    PM -->|CreateFsm| FSMInst
    FSMInst -->|管理状态| PB
    GFE -->|GetModule| PM
    GFE -->|GetModule| FSM
    
    style PM fill:#f9f,stroke:#333,stroke-width:2px
    style IPM fill:#ff9,stroke:#333,stroke-width:1px
```

### 组件Responsibility

| 组件 | Responsibility |
|------|------|
| **ProcedureManager** | 核心Implements类，Manages流程的Creates、切换、查询 |
| **IProcedureManager** | Interface Definitions，抽象流程Manages操作 |
| **ProcedureComponent** | Unity Component，used for在场景中configuration和Initialize流程 |
| **ProcedureBase** | Procedure Base，每个具体流程Inherits此类 |
| **IFsmManager** | 有限状态机Manages器（外部Depends on） |

## Core Features

### 1. Initialize流程Manages器

在Uses流程Manages器之前，需要先进行Initialize。Initialize时需要provides FSM Manages器和所有可用的流程Type。

```csharp
public void Initialize(IFsmManager fsmManager, params ProcedureBase[] procedures)
```

> Source: [ProcedureManager.cs](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureManager.cs#L104-L110)

**ParametersDescription：**
- `fsmManager`: 有限状态机Manages器，responsible forCreates和Manages FSM 实例
- `procedures`: 可用的流程实例数组

### 2. start流程

Initializecomplete后，需要指定一个入口流程来start流程Manages器。

```csharp
// 泛型方式
public void StartProcedure<T>() where T : ProcedureBase

// Type方式
public void StartProcedure(Type procedureType)
```

> Source: [ProcedureManager.cs](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureManager.cs#L116-L138)

### 3. 查询流程状态

provides多种方式查询当前流程information和check特定流程Whether exists。

```csharp
// 获取当前流程
public ProcedureBase CurrentProcedure { get; }

// 获取当前流程持续时间
public float CurrentProcedureTime { get; }

// 检查是否存在指定流程（泛型）
public bool HasProcedure<T>() where T : ProcedureBase

// 检查是否存在指定流程（Type）
public bool HasProcedure(Type procedureType)
```

> Source: [ProcedureManager.cs](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureManager.cs#L44-L71), [L145-L168](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureManager.cs#L145-L168)

### 4. get流程实例

可以在运行时get任意已register的流程实例进行操作。

```csharp
// 泛型方式获取
public ProcedureBase GetProcedure<T>() where T : ProcedureBase

// Type方式获取
public ProcedureBase GetProcedure(Type procedureType)
```

> Source: [ProcedureManager.cs](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureManager.cs#L175-L198)

## 流程切换流程

```mermaid
sequenceDiagram
    participant Unity as Unity引擎
    participant PC as ProcedureComponent
    participant PM as ProcedureManager
    participant FSM as IFsmManager
    participant PB as ProcedureBase

    Unity->>PC: Awake()
    PC->>PM: GameFrameworkEntry.GetModule&lt;IProcedureManager&gt;()
    PC->>PC: Start() 协程
    PC->>PM: Initialize(fsmManager, procedures)
    PM->>FSM: CreateFsm(this, procedures)
    FSM-->>PM: IFsm&lt;IProcedureManager&gt;
    PM-->>PC: 初始化完成
    PC->>PM: StartProcedure&lt;T&gt;()
    PM->>FSM: Start&lt;T&gt;()
    FSM->>PB: OnEnter() - 进入流程
    Note over FSM,PB: 流程运行中...
    PB->>FSM: 触发流程切换
    FSM->>PB: OnLeave() - 离开旧流程
    FSM->>PB: OnEnter() - 进入新流程
```

## Usage Examples

### 基本Uses流程

1. **CreatesCustom Procedures类**

```csharp
using GameFrameX.Procedure.Runtime;

public class LoginProcedure : ProcedureBase
{
    protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
    {
        base.OnEnter(procedureOwner);
        // 初始化登录流程
    }

    protected override void OnUpdate(IFsm<IProcedureManager> procedureOwner, float elapseSeconds, float realElapseSeconds)
    {
        base.OnUpdate(procedureOwner, elapseSeconds, realElapseSeconds);
        // 检查是否满足切换条件
        // 如果满足，切换到下一个流程
    }
}

public class GameProcedure : ProcedureBase
{
    protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
    {
        base.OnEnter(procedureOwner);
        // 开始游戏
    }
}
```

2. **configuration ProcedureComponent**

在 Unity 场景中add `ProcedureComponent` 组件，并configuration：
- `Available Procedure Type Names`: add所有流程的Type名称
- `Entrance Procedure Type Name`: set入口流程Type名称

3. **在代码中切换流程**

```csharp
// 假设在某个 Procedure 中切换到下一个流程
public class LoginProcedure : ProcedureBase
{
    protected override void OnUpdate(IFsm<IProcedureManager> procedureOwner, float elapseSeconds, float realElapseSeconds)
    {
        base.OnUpdate(procedureOwner, elapseSeconds, realElapseSeconds);
        
        // 登录完成后，切换到游戏流程
        if (IsLoginSuccess)
        {
            procedureOwner.ChangeState<GameProcedure>();
        }
    }
}
```

> Source: [ProcedureBase.cs](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureBase.cs#L1-L77)

### 直接Uses ProcedureManager

如果需要直接Uses ProcedureManager 而非through ProcedureComponent：

```csharp
// 获取流程管理器
IProcedureManager procedureManager = GameFrameworkEntry.GetModule<IProcedureManager>();

// 获取 FSM 管理器
IFsmManager fsmManager = GameFrameworkEntry.GetModule<IFsmManager>();

// 初始化
procedureManager.Initialize(fsmManager, new LoginProcedure(), new GameProcedure());

// 启动入口流程
procedureManager.StartProcedure<LoginProcedure>();

// 在游戏循环中查询
ProcedureBase current = procedureManager.CurrentProcedure;
float time = procedureManager.CurrentProcedureTime;
```

## API Reference

### Property

| Property | Type | Description |
|------|------|------|
| **CurrentProcedure** | `ProcedureBase` | get当前正在执行的流程实例 |
| **CurrentProcedureTime** | `float` | get当前流程已持续的时间（秒） |

### Method

#### `Initialize(IFsmManager fsmManager, params ProcedureBase[] procedures)`

Initialize流程Manages器。

**Parameters：**
- `fsmManager` (IFsmManager): 有限状态机Manages器
- `procedures` (params ProcedureBase[]): 可用的流程实例数组

**Exception：**
- `GameFrameworkException`: 当 fsmManager 为 null 时抛出

---

#### `StartProcedure<T>() where T : ProcedureBase`

开始指定的流程（泛型方式）。

**Parameters：**
- 无

**Exception：**
- `GameFrameworkException`: 当流程Manages器未Initialize时抛出

---

#### `StartProcedure(Type procedureType)`

开始指定的流程（Type 方式）。

**Parameters：**
- `procedureType` (Type): 要开始的流程Type

**Exception：**
- `GameFrameworkException`: 当流程Manages器未Initialize时抛出

---

#### `HasProcedure<T>() where T : ProcedureBase`

Check if exists指定的流程。

**返回：** 
- `bool`: 如果Returns true if it exists; otherwise returns false

**Exception：**
- `GameFrameworkException`: 当流程Manages器未Initialize时抛出

---

#### `GetProcedure<T>() where T : ProcedureBase`

get指定Type的流程实例。

**返回：** 
- `ProcedureBase`: 流程实例

**Exception：**
- `GameFrameworkException`: 当流程Manages器未Initialize时抛出

---

### 生命周期Method

ProcedureManager Inherits自 `GameFrameworkModule`，主要生命周期Method：

| Method | Description |
|------|------|
| `Update(elapseSeconds, realElapseSeconds)` | 轮询Method，当前Implements为空（由 FSM 自行process） |
| `Shutdown()` | Closed并cleanup流程Manages器，destroy FSM |

> Source: [ProcedureManager.cs](https://github.com/GameFrameX/com.gameframex.unity.procedure/blob/main/Runtime/Procedure/ProcedureManager.cs#L78-L97)

## Dependencies

ProcedureManager Depends on以下组件：

| Depends on项 | Description | 来源 |
|--------|------|------|
| **IFsmManager** | 有限状态机Manages器 | GameFrameX.Fsm.Runtime |
| **IFsm\<IProcedureManager\>** | 流程状态机实例 | GameFrameX.Fsm.Runtime |
| **GameFrameworkModule** | GameFramework 模块基类 | GameFrameX.Runtime |
| **ProcedureBase** | Procedure Base | 本模块 |

## Related Links

- [IProcedureManager](./3-core-concepts.iprocedure-manager) - 流程Manages器Interface Definitions
- [ProcedureBase](./3-core-concepts.procedure-base) - Procedure Base
- [ProcedureComponent](./3-core-concepts.procedure-component) - Unity Procedure Component
- [FSM 模块](https://github.com/GameFrameX/com.alianblank.gameframex.unity.fsm) - 有限状态机模块
- [Custom Procedures](./4-custom-procedures) - 如何CreatesCustom Procedures
