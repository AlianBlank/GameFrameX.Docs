# 声音管理器

声音管理器（SoundManager）是 GameFrameX 音频模块的核心组件，负责管理游戏中的声音播放、声音组、声音代理以及资源加载等核心功能。它提供了统一的接口来控制背景音乐、音效、语音等各种音频资源，支持优先级管理、音量控制、淡入淡出等高级特性。

## 概述

SoundManager 是 GameFrameX 框架中处理所有音频相关操作的核心模块。作为 GameFrameworkModule 的实现类，它遵循框架的模块化设计原则，可以与其他框架模块（如资源管理器、事件系统）无缝集成。

**核心职责：**
- 管理多个声音组（SoundGroup），每个组可独立控制音量和静音状态
- 协调声音代理（SoundAgent）的分配，实现优先级调度和资源复用
- 异步加载音频资源，支持 YooAsset 资源管理系统
- 触发播放成功/失败事件，便于游戏逻辑响应音频状态变化

**设计意图：**
SoundManager 采用分层架构设计，将音频播放的底层实现（SoundAgent）与管理层（SoundManager）分离。这种设计允许开发者通过自定义 Helper 来适配不同的音频引擎（如 Unity AudioSource、Fmod、AudioKit），同时保持上层代码的一致性。优先级机制确保重要音效能够及时播放，避免因代理池满导致的声音丢失。

```mermaid
flowchart TD
    subgraph SoundManager["声音管理器 (SoundManager)"]
        SM[核心管理器]
        SG[声音组字典]
        SL[加载中的声音列表]
        SR[待释放资源集合]
    end
    
    subgraph External["外部依赖"]
        AM[资源管理器]
        SH[声音辅助器]
        EC[事件组件]
    end
    
    subgraph SoundGroup["声音组 (SoundGroup)"]
        SGH[辅助器]
        SAG[声音代理列表]
    end
    
    subgraph SoundAgent["声音代理 (SoundAgent)"]
        SAH[代理辅助器]
        ASSET[音频资源]
    end
    
    AM --> SM : 加载音频资源
    SH --> SM : 释放资源回调
    SM --> SG : 管理声音组
    SG --> SGH
    SGH --> SAG
    SAG --> SAH
    SAH --> ASSET : 播放音频
    SM --> EC : 触发播放事件
```

## 核心架构

### 模块层次结构

SoundManager 采用三层架构设计，每层承担不同的职责：

1. **接口层**：定义 ISoundManager、ISoundGroup、ISoundAgent 等接口，封装业务逻辑
2. **实现层**：SoundManager、SoundGroup、SoundAgent 的具体实现
3. **辅助层**：各种 Helper 接口及其默认实现，提供平台特定的音频处理能力

```mermaid
classDiagram
    class ISoundManager {
        <<interface>>
        +SoundGroupCount: int
        +PlaySoundSuccess: event
        +PlaySoundFailure: event
        +SetResourceManager()
        +SetSoundHelper()
        +AddSoundGroup()
        +PlaySound()
        +StopSound()
        +PauseSound()
        +ResumeSound()
    }
    
    class ISoundGroup {
        <<interface>>
        +Name: string
        +SoundAgentCount: int
        +Mute: bool
        +Volume: float
    }
    
    class ISoundAgent {
        <<interface>>
        +SerialId: int
        +IsPlaying: bool
        +Length: float
        +Time: float
        +Loop: bool
        +Priority: int
        +Volume: float
        +Play()
        +Stop()
        +Pause()
        +Resume()
    }
    
    class ISoundHelper {
        <<interface>>
        +ReleaseSoundAsset()
    }
    
    class ISoundAgentHelper {
        <<interface>>
        +IsPlaying: bool
        +Length: float
        +Mute: bool
        +Loop: bool
        +Priority: int
        +Volume: float
        +Play()
        +Stop()
    }
    
    class ISoundGroupHelper {
        <<interface>>
    }
    
    class SoundManager {
        -m_SoundGroups: Dictionary~string, SoundGroup~
        -m_SoundsBeingLoaded: List~int~
        -m_AssetManager: IAssetManager
        -m_SoundHelper: ISoundHelper
    }
    
    class SoundGroup {
        -m_Name: string
        -m_SoundGroupHelper: ISoundGroupHelper
        -m_SoundAgents: List~SoundAgent~
        -m_Mute: bool
        -m_Volume: float
    }
    
    class SoundAgent {
        -m_SoundGroup: SoundGroup
        -m_SoundHelper: ISoundHelper
        -m_SoundAgentHelper: ISoundAgentHelper
        -m_SerialId: int
    }
    
    ISoundManager <|.. SoundManager
    ISoundGroup <|.. SoundGroup
    ISoundAgent <|.. SoundAgent
    SoundManager --> SoundGroup : 管理
    SoundGroup --> SoundAgent : 分配
    SoundAgent --> ISoundHelper : 使用
    SoundAgent --> ISoundAgentHelper : 使用
    SoundGroup --> ISoundGroupHelper : 使用
```

### 核心数据结构

SoundManager 内部维护三个核心数据结构：

```mermaid
erDiagram
    SOUND_MANAGER ||--o{ SOUND_GROUP : "管理"
    SOUND_GROUP ||--o{ SOUND_AGENT : "包含"
    SOUND_AGENT ||--o{ AUDIO_CLIP : "播放"
    
    SOUND_MANAGER {
        string name
        Dictionary_SoundGroup soundGroups
        List_int soundsBeingLoaded
        HashSet_int soundsToReleaseOnLoad
    }
    
    SOUND_GROUP {
        string name
        bool avoidBeingReplaced
        bool mute
        float volume
    }
    
    SOUND_AGENT {
        int serialId
        bool isPlaying
        bool loop
        int priority
        float volumeInGroup
    }
    
    AUDIO_CLIP {
        string assetName
        float duration
    }
```

- **m_SoundGroups**：存储所有声音组的字典，以组名为键，支持 O(1) 查找
- **m_SoundsBeingLoaded**：记录正在异步加载的声音序列 ID，用于跟踪加载状态
- **m_SoundsToReleaseOnLoad**：记录需要在下一次加载完成时立即释放的声音 ID，用于处理 StopSound 在加载期间被调用的情况

## 声音播放流程

### 完整播放流程

当调用 PlaySound 方法时，SoundManager 会执行以下步骤：

```mermaid
sequenceDiagram
    participant Client as 客户端代码
    participant SM as SoundManager
    participant RM as ResourceManager
    participant SG as SoundGroup
    participant SA as SoundAgent
    participant Event as 事件系统
    
    Client->>SM: PlaySound(soundAssetName, soundGroupName, playSoundParams)
    
    rect rgb(240, 248, 255)
        Note over SM: 1. 参数验证
    end
    
    SM->>SM: 验证资源管理器、声音辅助器、声音组
    
    rect rgb(255, 250, 240)
        Note over SM: 2. 分配序列编号
    end
    
    alt 使用指定序列ID
        SM->>SM: 使用传入的 serialId
    else 自动生成
        SM->>SM: ++m_Serial 生成新ID
    end
    
    rect rgb(240, 255, 240)
        Note over SM,RM: 3. 异步加载资源
    end
    
    SM->>RM: LoadAssetAsync<AudioClip>(soundAssetName)
    RM-->>SM: 返回 AssetOperationHandle
    
    SM->>SM: 将 serialId 加入 m_SoundsBeingLoaded
    
    rect rgb(255, 245, 238)
        Note over SM: 4. 加载完成回调
    end
    
    RM->>SM: OnAssetOperationHandleOnCompleted()
    
    alt 资源需要释放（已被StopSound取消）
        SM->>SM: 释放资源，不触发事件
    else 正常加载
        SM->>SG: PlaySound(serialId, audioClip, params)
        
        rect rgb(230, 230, 250)
            Note over SG: 5. 选择声音代理
        end
        
        SG->>SG: 查找可用代理或低优先级代理
        
        alt 找到可用代理
            SG->>SA: 设置音频资源
            
            rect rgb(250, 250, 210)
                Note over SA: 6. 播放声音
            end
            
            SA->>SA: 应用播放参数并开始播放
            
            SM-->>Event: 触发 PlaySoundSuccess 事件
            SM-->>Client: 返回 serialId
        else 无可用代理（优先级过低）
            SG-->>SM: 返回 null
            SM-->>Event: 触发 PlaySoundFailure 事件
            SM-->>Client: 返回 serialId
        end
    end
```

### 声音代理选择策略

SoundGroup 采用智能代理选择策略，确保高优先级声音能够及时播放：

```mermaid
flowchart TD
    Start([接收到播放请求]) --> CheckFree{有空闲代理？}
    
    CheckFree -->|是| UseFree[使用空闲代理播放]
    CheckFree -->|否| CheckPriority{请求优先级 > 最低代理优先级？}
    
    CheckPriority -->|是| FindLowest[查找最低优先级代理]
    CheckPriority -->|否| CheckSamePriority{允许同优先级替换？}
    
    FindLowest --> Replace[替换低优先级代理]
    
    CheckSamePriority -->|是| FindOldest[查找最早播放的同优先级代理]
    CheckSamePriority -->|否| Reject[返回错误：优先级过低]
    
    FindOldest --> Replace
    
    UseFree --> Done([播放成功])
    Replace --> Done
    Reject --> Fail([播放失败])
```

**代理选择逻辑：**
1. 优先使用空闲的代理
2. 如果没有空闲代理，比较请求优先级与现有播放声音的优先级
3. 如果请求优先级更高，替换最低优先级的代理
4. 如果优先级相同，根据 `AvoidBeingReplacedBySamePriority` 设置决定是否替换同优先级声音

## 使用示例

### 基础使用方式

在 Unity 场景中添加 SoundComponent 后，可以通过以下方式播放声音：

```csharp
// 通过 SoundComponent 播放音效
var soundComponent = GameEntry.GetComponent<SoundComponent>();
int serialId = await soundComponent.PlaySound("Assets/Sounds/click.wav", "UI");
```

> Source: [SoundComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/Sound/SoundComponent.cs#L356-L359)

### 带参数的播放

```csharp
// 创建播放参数
var playParams = PlaySoundParams.Create(false);
playParams.VolumeInSoundGroup = 0.8f;
playParams.Priority = 100;
playParams.Loop = false;
playParams.FadeInSeconds = 0.3f;

// 播放音效
int serialId = await soundComponent.PlaySound("Assets/Sounds/bgm_main.wav", "Background", playParams);
```

> Source: [PlaySoundParams.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/Sound/Sound/PlaySoundParams.cs#L233-L239)

### 事件监听

```csharp
// 监听播放成功事件
soundComponent.PlaySoundSuccess += OnPlaySoundSuccess;

// 监听播放失败事件
soundComponent.PlaySoundFailure += OnPlaySoundFailure;

private void OnPlaySoundSuccess(object sender, PlaySoundSuccessEventArgs e)
{
    Debug.Log($"声音播放成功: {e.SoundAssetName}, SerialId: {e.SerialId}");
    // 可通过 e.SoundAgent 获取播放代理进行更精细的控制
}

private void OnPlaySoundFailure(object sender, PlaySoundFailureEventArgs e)
{
    Debug.LogWarning($"声音播放失败: {e.SoundAssetName}, 错误码: {e.ErrorCode}, 原因: {e.ErrorMessage}");
}
```

> Source: [PlaySoundSuccessEventArgs.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/EventArgs/PlaySoundSuccessEventArgs.cs#L89-L98)

### 声音控制

```csharp
// 暂停指定声音
soundComponent.PauseSound(serialId);

// 恢复播放
soundComponent.ResumeSound(serialId);

// 停止播放（带淡出效果）
soundComponent.StopSound(serialId, 0.5f);

// 停止所有已加载的声音
soundComponent.StopAllLoadedSounds(1.0f);

// 停止所有正在加载的声音
soundComponent.StopAllLoadingSounds();
```

> Source: [SoundManager.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/Sound/Sound/SoundManager.cs#L584-L609)

### 声音组管理

```csharp
// 添加新的声音组
soundComponent.AddSoundGroup("Combat", false, false, 1.0f, 4);

// 获取声音组
var soundGroup = soundComponent.GetSoundGroup("UI");

// 设置组音量
soundGroup.Volume = 0.5f;

// 设置组静音
soundGroup.Mute = true;

// 检查声音组是否存在
if (soundComponent.HasSoundGroup("Background"))
{
    // 播放背景音乐
}
```

> Source: [SoundComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/Sound/SoundComponent.cs#L199-L212)

## 默认常量值

SoundManager 使用以下默认常量值，可以在 Constant.cs 中查看：

> Source: [Constant.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/Sound/Sound/Constant.cs#L38-L99)

| 属性 | 默认值 | 说明 |
|------|--------|------|
| DefaultTime | 0f | 默认播放位置 |
| DefaultMute | false | 默认不静音 |
| DefaultLoop | false | 默认不循环 |
| DefaultPriority | 0 | 默认优先级 |
| DefaultVolume | 1f | 默认音量（满音量） |
| DefaultFadeInSeconds | 0f | 默认淡入时间 |
| DefaultFadeOutSeconds | 0f | 默认淡出时间 |
| DefaultPitch | 1f | 默认音调 |
| DefaultPanStereo | 0f | 默认立体声位置 |
| DefaultSpatialBlend | 0f | 默认空间混合量 |
| DefaultMaxDistance | 100f | 默认最大距离 |
| DefaultDopplerLevel | 1f | 默认多普勒等级 |

## 错误码说明

播放声音时可能遇到以下错误码：

> Source: [PlaySoundErrorCode.cs](https://github.com/GameFrameX/com.gameframex.unity.sound/blob/main/Runtime/Sound/Sound/PlaySoundErrorCode.cs#L38-L69)

| 错误码 | 说明 |
|--------|------|
| Unknown | 未知错误 |
| SoundGroupNotExist | 指定的声音组不存在 |
| SoundGroupHasNoAgent | 声音组没有可用的声音代理 |
| LoadAssetFailure | 加载音频资源失败 |
| IgnoredDueToLowPriority | 由于优先级过低被忽略 |
| SetSoundAssetFailure | 设置音频资源到代理失败 |

## 相关链接

- [声音组](./4-core-concepts.2-sound-group) - 了解声音组的详细配置和管理
- [声音代理](./4-core-concepts.3-sound-agent) - 了解声音代理的工作原理
- [接口定义](./5-api-reference.1-interfaces) - 查看完整的接口定义
- [播放参数](./5-api-reference.2-play-sound-params) - 查看 PlaySoundParams 详细参数
- [事件参数](./5-api-reference.3-event-args) - 查看事件参数定义
- [配置说明](./6-configuration) - 查看模块配置说明
- [扩展指南](./7-extending) - 了解如何扩展音频系统
