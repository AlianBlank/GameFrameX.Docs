# 快速开始

GameFrameX Unity Builder は機能强大的自动化构建系统，专门に使用 Unity プロジェクト的持续集成和自动化打包。を通じて命令行参数驱动，该系统能够自动完成从环境准备、リソース打包到 APK 构建的全フロー。

## 概要

GameFrameX Builder 是 GameFrameX ゲームフレームワーク的コアコンポーネント之一，专注于解决 Unity プロジェクト自动化构建的痛点。它を通じて解析命令行参数来配置构建フロー，サポート以下コア機能：

- **打包准备（BuildReady）**：自动检查并安装 HybridCLR ホットアップデートフレームワーク，执行代码生成
- **リソース打包（BuildAsset）**：使用 YooAsset 构建ゲームリソース包，サポート增量构建
- **APK 构建（BuildApk）**：自动构建 Android 安装包
- **オブジェクトストレージ上传**：サポート腾讯云、阿里云、七牛云等主流云存储
- **版本更新通知**：自动更新リソース包版本并を通じて企业微信发送通知

## アーキテクチャ概要

```mermaid
flowchart TD
    subgraph CLI["命令行参数"]
        CMD[命令行输入]
    end

    subgraph Core["构建コア"]
        Parse[参数解析]
        Options[BuilderOptions]
    end

    subgraph Prepare["准备阶段"]
        Ready[BuildReady]
        HybridCLR[HybridCLR检查安装]
        Refresh[AssetDatabase刷新]
        Generate[代码生成]
    end

    subgraph Build["构建阶段"]
        Asset[BuildAsset]
        CopyHotfix[复制热更代码]
        CopyAOT[复制AOT代码]
        YooAsset[YooAsset打包]
    end

    subgraph Package["发布阶段"]
        Apk[BuildApk]
        Upload[オブジェクトストレージ上传]
        VersionUpdate[版本更新]
        WeChat[企业微信通知]
    end

    CLI --> Parse
    Parse --> Options
    Options --> Ready
    Ready --> HybridCLR
    Ready --> Refresh
    Ready --> Generate
    Options --> Asset
    Asset --> CopyHotfix
    Asset --> CopyAOT
    Asset --> YooAsset
    Options --> Apk
    Apk --> Upload
    YooAsset --> VersionUpdate
    VersionUpdate --> WeChat
```

## コアフロー

### 构建フロー序列图

```mermaid
sequenceDiagram
    participant CI as CI/CD系统
    participant Builder as Builderクラス
    participant HybridCLR as HybridCLR
    participant YooAsset as YooAsset
    participant Storage as オブジェクトストレージ
    participant Server as 版本服务器
    participant WeChat as 企业微信

    CI->>Builder: 执行命令行构建
    Builder->>Builder: 解析命令行参数
    
    alt 执行BuildReady
        Builder->>HybridCLR: 检查并安装HybridCLR
        HybridCLR-->>Builder: 安装完成
        Builder->>Builder: 刷新AssetDatabase
        Builder->>HybridCLR: 生成热更代码
    end
    
    alt 执行BuildAsset
        Builder->>Builder: 复制热更代码到ディレクトリ
        Builder->>Builder: 复制AOT代码到ディレクトリ
        Builder->>YooAsset: 构建リソース包
        YooAsset-->>Builder: 构建完成
    end
    
    alt 执行BuildApk
        Builder->>Builder: 构建Android APK
    end
    
    alt 上传リソース
        Builder->>Storage: 上传リソース包到云存储
    end
    
    alt 更新版本
        Builder->>Server: 上报版本信息
        Server-->>Builder: 更新成功
        Builder->>WeChat: 发送构建通知
    end
    
    Builder-->>CI: 构建完成
```

## 命令行参数

### 必須参数

| 参数 | 説明 | 示例 |
|------|------|------|
| `-executeMethod` | 执行メソッド | `GameFrameX.Builder.Editor.Builder.BuildAsset` |
| `-BUILD_NUMBER` | 构建号 | `1001` |

### 构建配置参数

| 参数 | 説明 | 默认值 |
|------|------|--------|
| `-JOB_NAME` | 任务名称 | - |
| `-PackageName` | リソース包名称 | `DefaultPackage` |
| `-ChannelName` | 渠道名称 | `default` |
| `-BundleId` | 包名 | プロジェクト默认值 |
| `-AppVersion` | 程序版本号 | プロジェクト默认值 |
| `-IsIncrementalBuildPackage` | 是否增量构建 | `false` |
| `-Language` | 语言 | `default` |

### オブジェクトストレージ参数

| 参数 | 説明 |
|------|------|
| `-ObjectStorageKey` | オブジェクトストレージAccessKey |
| `-ObjectStorageSecret` | オブジェクトストレージSecretKey |
| `-ObjectStorageBucketName` | バケット名 |
| `-ObjectStorageEndPoint` | 区域节点 |

### 上传控制参数

| 参数 | 説明 |
|------|------|
| `-IsUploadLogFile` | 是否上传日志ファイル |
| `-IsUploadAsset` | 是否上传リソース包 |
| `-IsUploadApk` | 是否上传APK |
| `-IsUpdateAssetPackageVersion` | 是否更新リソース包版本 |
| `-UpdateAssetPackageVersionUrl` | 版本更新服务器URL |
| `-UpdateAssetPackageVersionAuthorization` | 版本更新授权码 |
| `-WeChatBotKey` | 企业微信机器人Key |

## 使用例

### Jenkins 构建任务配置

```groovy
// Jenkinsfile 示例
pipeline {
    agent any
    stages {
        stage('构建') {
            steps {
                bat '''
                    Unity.exe -projectPath "${WORKSPACE}" ^
                    -executeMethod GameFrameX.Builder.Editor.Builder.BuildAsset ^
                    -BUILD_NUMBER "${BUILD_NUMBER}" ^
                    -JOB_NAME "${JOB_NAME}" ^
                    -PackageName "GamePackage" ^
                    -ChannelName "official" ^
                    -IsUploadLogFile true ^
                    -IsUploadAsset true ^
                    -logFile "../Logs/build_${BUILD_NUMBER}.log"
                '''
            }
        }
    }
}
```

> Source: [Builder.cs](https://github.com/GameFrameX/com.gameframex.unity.builder/blob/main/Editor/Builder.cs#L60-L181)

### 完整的构建命令示例

```bash
Unity.exe -projectPath "D:\UnityProject\MyGame" ^
-executeMethod GameFrameX.Builder.Editor.Builder.BuildAsset ^
-BUILD_NUMBER "1001" ^
-JOB_NAME "DailyBuild" ^
-PackageName "GamePackage" ^
-ChannelName "official" ^
-Language "zh-CN" ^
-ObjectStorageKey "your_access_key" ^
-ObjectStorageSecret "your_secret_key" ^
-ObjectStorageBucketName "game-assets" ^
-ObjectStorageEndPoint "ap-guangzhou" ^
-IsUploadLogFile true ^
-IsUploadAsset true ^
-IsUpdateAssetPackageVersion true ^
-UpdateAssetPackageVersionUrl "https://api.example.com/version" ^
-UpdateAssetPackageVersionAuthorization "Bearer token" ^
-WeChatBotKey "your_wechat_key" ^
-logFile "../Logs/build_1001.log"
```

### 不同构建阶段

に基づいて需求选择不同的执行メソッド：

```csharp
// 打包准备 - 安装HybridCLR、生成热更代码
-executeMethod GameFrameX.Builder.Editor.Builder.BuildReady

// 打包リソース - 构建リソース包
-executeMethod GameFrameX.Builder.Editor.Builder.BuildAsset

// 构建APK - 打包安装包
-executeMethod GameFrameX.Builder.Editor.Builder.BuildApk
```

> Source: [Builder.cs](https://github.com/GameFrameX/com.gameframex.unity.builder/blob/main/Editor/Builder.cs#L183-L345)

## 构建选项配置

BuilderOptions クラス封装了所有构建配置选项：

```csharp
public class BuilderOptions
{
    // 日志ファイル路径
    public string LogFilePath { get; set; }
    
    // 执行メソッド
    public string ExecuteMethod { get; set; }
    
    // 构建号
    public string BuildNumber { get; set; }
    
    // オブジェクトストレージ配置
    public string ObjectStorageKey { get; set; }
    public string ObjectStorageSecret { get; set; }
    public string ObjectStorageBucketName { get; set; }
    public string ObjectStorageEndPoint { get; set; }
    
    // 构建配置
    public string JobName { get; set; }
    public string ChannelName { get; set; } = "default";
    public string BundleId { get; set; }
    public string AppVersion { get; set; }
    public string PackageName { get; set; } = "DefaultPackage";
    public bool IsIncrementalBuildPackage { get; set; }
    
    // 上传控制
    public bool IsUploadLogFile { get; set; }
    public bool IsUploadAsset { get; set; }
    public bool IsUploadApk { get; set; }
    public bool IsUpdateAssetPackageVersion { get; set; }
    
    // 版本更新
    public string UpdateAssetPackageVersionUrl { get; set; }
    public string UpdateAssetPackageVersionAuthorization { get; set; }
    
    // 通知
    public string Language { get; set; } = "default";
    public string WeChatBotKey { get; set; }
}
```

> Source: [BuilderOptions.cs](https://github.com/GameFrameX/com.gameframex.unity.builder/blob/main/Editor/BuilderOptions.cs#L5-L116)

## 機能特性説明

### ホットアップデートサポート

构建系统深度集成 HybridCLR，サポート：
- 自动检查 HybridCLR 安装ステータス
- 自动安装和配置 HybridCLR
- 自动生成ホットアップデート代码

### リソース打包

使用 YooAsset 进行リソース打包：
- サポート增量构建和全量构建
- 自动复制热更代码和 AOT 代码
- サポートリソース包版本号自动生成

### オブジェクトストレージ

サポート多种云存储服务：

| 云服务商 | 条件编译符号 | 説明 |
|---------|-------------|------|
| 腾讯云 | `ENABLE_GAME_FRAME_X_OBJECT_STORAGE_TENCENT` | 腾讯云COS |
| 阿里云 | `ENABLE_GAME_FRAME_X_OBJECT_STORAGE_A_LI_YUN` | 阿里云OSS |
| 七牛云 | `ENABLE_GAME_FRAME_X_OBJECT_STORAGE_QI_NIU` | 七牛云Kodo |

> Source: [Builder.cs](https://github.com/GameFrameX/com.gameframex.unity.builder/blob/main/Editor/Builder.cs#L40-L53)

### 版本更新通知

构建完成后自动：
1. 将版本信息上报到版本服务器
2. を通じて企业微信发送构建通知

> Source: [BuilderUpdateAssetPackageVersionHelper.cs](https://github.com/GameFrameX/com.gameframex.unity.builder/blob/main/Editor/BuilderUpdateAssetPackageVersionHelper.cs#L51-L85)
> Source: [WeChatNotifyWorkHelper.cs](https://github.com/GameFrameX/com.gameframex.unity.builder/blob/main/Editor/WeChatNotifyWorkHelper.cs#L45-L71)

## 依赖项

本构建系统依赖以下コアコンポーネント：

| 依赖包 | 説明 |
|--------|------|
| GameFrameX.Editor | 编辑器工具基クラス |
| GameFrameX.Runtime | 运行时基础库 |
| YooAsset | リソース管理包 |
| HybridCLR | IL2CPPホットアップデート方案 |

## 関連リンク

- [安装指南](./2-installation)
- [使用メソッド](./4-usage)
- [配置选项](./5-configuration)
- [オブジェクトストレージ配置](./6-object-storage)
- [常见问题](./7-troubleshooting)
- [GitHub 仓库](https://github.com/GameFrameX/com.gameframex.unity.builder.git)
- [官方文档](https://gameframex.doc.alianblank.com)
