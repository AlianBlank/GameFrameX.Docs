...
本文档将帮助您快速上手 GameFrameX Network 网络组件，了解其核心概念并开始构建您的网络应用程序。

## 概述

GameFrameX Network 是 GameFrameX 游戏框架的核心网络组件，提供 TCP 长连接功能，支持：
- 基于 TCP 的可靠连接
...
首先，需要定义您的请求和响应消息类型。消息必须继承自 `MessageObject` 类：
...
以下是一个完整的客户端示例，展示了从连接到处发送消息的完整流程：
...
| UniqueId | 4 字节 | 消息唯一编号，用于 RPC 匹配 |
| MessageId | 4 字节 | 消息类型 ID |
...
- [概述](./1-overview) - 了解更多关于网络组件的整体设计
- [安装](./2-installation) - 安装和配置网络组件
...
- [消息类型](./4-core-concepts.3-message-types) - 消息类型系统详解
- [数据包处理器](./4-core-concepts.4-packet-handlers) - 数据包处理机制
- [心跳机制](./5-heartbeat) - 心跳检测机制详解
- [事件系统](./6-events) - 网络事件系统
- [消息压缩](./7-message-compression) - 消息压缩配置
- [RPC 远程调用](./8-rpc) - RPC 调用机制
- [常见问题](./10-troubleshooting) - 常见问题与解决方案
