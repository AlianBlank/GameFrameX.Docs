...
本节将帮助您快速上手使用 GameFrameX Get Channel 插件，在 Unity 项目中获取多平台渠道号。

## 概述

GameFrameX Get Channel 是一个轻量级的 Unity 插件，专门用于在游戏或应用分发时获取渠道标识信息。无论您的项目是发布到 iOS、Android，还是 PC、WebGL 等平台，都可以使用统一的 API 获取渠道号，无需编写平台特定的代码。
...
将以下依赖添加到您项目的 `Packages/manifest.json` 中：
...
> **提示**：详细的安装说明请参阅 [安装](./2-installation) 文档。

### 第二步：配置渠道信息

根据您的目标平台，在对应位置配置渠道信息：

| 目标平台 | 配置文件 | 配置方式 |
|----------|----------|----------|
| iOS / tvOS / visionOS | Info.plist | 添加 `channel` 键值对 |
| Android | AndroidManifest.xml | 在 application 标签添加 meta-data |
| Editor / PC / WebGL / UWP | Resources/application_config.txt | 创建文本文件，每行 `key=value` |

**详细配置方法请参阅 [平台配置](./5-platform-configuration) 文档。**

### 第三步：调用 API 获取渠道号

在任意 C# 脚本中调用 `BlankGetChannel.GetChannelName()` 方法：
...
在 Unity 编辑器中运行游戏（使用 Editor 平台），或者构建到目标平台，检查日志输出是否正确显示渠道号。

## 工作原理

当您调用 `GetChannelName()` 方法时，插件会按照以下流程获取渠道信息：
...
**关键设计说明：**
- **缓存机制**：使用 `Dictionary<string, string>` 存储已读取的渠道信息，避免每次调用都读取配置文件，提高性能
- **平台判断**：通过 Unity 预处理器指令（`UNITY_ANDROID`、`UNITY_IOS` 等）判断当前平台
- **默认值保护**：当指定的键不存在时，返回调用方指定的默认值，确保程序正常运行

## 完整示例

以下是一个完整的 MonoBehaviour 示例，展示如何在游戏启动时获取并使用渠道信息：
...
1. **键名一致性**：确保代码中使用的键名与平台配置文件中的键名完全一致
2. **Android 原生代码**：Android 平台需要确保项目中已集成 `com.alianhome.getchannel` 原生模块（插件包中已包含）
3. **iOS Info.plist**：首次构建 iOS 时，若未手动配置，插件会自动添加默认渠道 `channel=default`

## 相关链接

- [安装指南](./2-installation) - 详细的插件安装方法
- [API 参考](./4-api-reference) - 完整的 API 方法说明
- [平台配置](./5-platform-configuration) - 各平台详细配置说明
- [示例项目](./6-samples) - 更多使用示例
