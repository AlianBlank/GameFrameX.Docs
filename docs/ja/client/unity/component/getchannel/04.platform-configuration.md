# 平台配置

本章节详细介绍 GameFrameX GetChannel 插件在不同平台上的配置メソッド，包括 iOS/tvOS/visionOS、Android、Editor、PC、WebGL、UWP 和主机平台（PS4、PS5、Xbox One、Nintendo Switch）的配置ファイル設定。

## 概要

GameFrameX GetChannel 插件を通じて统一的 API インターフェース取得各平台的渠道信息。为了実装跨平台サポート，每个平台都有其特定的数据来源和读取方法：

| 平台 | 配置源 | 读取方法 |
|------|--------|----------|
| iOS/tvOS/visionOS | Info.plist | 原生 Objective-C++ メソッド呼び出し |
| Android | AndroidManifest.xml | Android Java 反射呼び出し |
| Editor/PC/WebGL/UWP/Console | application_config.txt | Unity Resources 加载 |

## アーキテクチャ設計

```mermaid
graph TD
    subgraph Unity["Unity 运行时"]
        API["BlankGetChannel.cs<br/>统一 API インターフェース"]
        Cache["ChannelCache<br/>内存缓存"]
    end

    subgraph iOS["iOS/tvOS/visionOS 平台"]
        ObjC["BlankGetChannel.mm<br/>Objective-C++ 原生代码"]
        Plist["Info.plist<br/>配置ファイル"]
    end

    subgraph Android["Android 平台"]
        Java["com.alianhome.getchannel.jar<br/>Java 原生代码"]
        Manifest["AndroidManifest.xml<br/>清单ファイル"]
    end

    subgraph Other["其他平台"]
        Config["application_config.txt<br/>文本配置ファイル"]
    end

    API -->|1. 检查缓存| Cache
    API -->|2. 平台特定读取| ObjC
    API -->|3. 平台特定读取| Java
    API -->|4. Resources 加载| Config
    
    ObjC -->|读取| Plist
    Java -->|读取| Manifest
    
    Cache -.->|命中则返回| API
    ObjC -.->|返回结果| API
    Java -.->|返回结果| API
    Config -.->|返回结果| API
```

### コンポーネント説明

| コンポーネント | 位置 | 职责 |
|------|------|------|
| BlankGetChannel | Runtime/BlankGetChannel.cs | 提供统一的 C# API，管理缓存和平台分支 |
| PostProcessBuildHandler | Editor/PostProcessBuildHandler.cs | iOS 构建后自动添加默认渠道配置 |
| BlankGetChannel.mm | Plugins/iOS/BlankGetChannel/ | iOS 平台原生実装，读取 Info.plist |
| com.alianhome.getchannel.jar | Plugins/Android/ | Android 平台原生実装，读取 meta-data |

## iOS/tvOS/visionOS 平台配置

### Info.plist 配置

在 Xcode プロジェクト的 `Info.plist` ファイル中添加渠道信息。使用 **String** クラス型键值对：

```xml
<!-- 必选：主渠道 -->
<key>channel</key>
<string>ios_cn_taptap</string>

<!-- 可选：子渠道 -->
<key>sub_channel</key>
<string>beta</string>

<!-- 可选：其他自定义键 -->
<key>app_id</key>
<string>123456789</string>
```
> Source: [README.md](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/README.md#L96-L104)

### 自动默认配置

插件包含构建后处理器 `PostProcessBuildHandler.cs`，会在 iOS 构建时自动执行以下逻辑：

```csharp
// PostProcessBuildHandler.cs コア逻辑
const string channelKey = "channel";
const string channelValue = "default";

string plistPath = path + "/Info.plist";
PlistDocument plist = new PlistDocument();
plist.ReadFromString(File.ReadAllText(plistPath));
if (!plist.root.values.TryGetValue(channelKey, out var value))
{
    plist.root.SetString(channelKey, channelValue);
    plist.WriteToFile(plistPath);
    Debug.Log("配置渠道成功=>" + channelValue);
}
```
> Source: [PostProcessBuildHandler.cs](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/Editor/PostProcessBuildHandler.cs#L31-L48)

**自动配置规则：**
- 如果 `Info.plist` 中**不存在** `channel` 键 → 自动添加 `channel = "default"`
- 如果 `Info.plist` 中**已存在** `channel` 键 → 跳过修改，保留原值

### iOS 原生実装

iOS 平台を通じて Objective-C++ 実装渠道读取：

```cpp
// Plugins/iOS/BlankGetChannel/BlankGetChannel.mm
char * getChannelName(char * channel_name) {
    NSString* channel_name_String = [NSString stringWithUTF8String:channel_name];
    NSString* File = [[NSBundle mainBundle] pathForResource:@"Info" ofType:@"plist"];
    NSMutableDictionary* dict = [[NSMutableDictionary alloc] initWithContentsOfFile:File];
    NSString * channel_value = [dict objectForKey:channel_name_String];
    if (channel_value != nil) {
        return StringToCharArray(channel_value);
    }
    channel_value = @"unknown";
    return StringToCharArray(channel_value);
}
```
> Source: [BlankGetChannel.mm](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/Plugins/iOS/BlankGetChannel/BlankGetChannel.mm#L25-L35)

## Android 平台配置

### AndroidManifest.xml 配置

在 `AndroidManifest.xml` 的 `<application>` 标签内添加 `<meta-data>` 标签：

```xml
<application ...>
    <activity ...>
        <!-- 活动配置 -->
    </activity>

    <!-- 必选：主渠道 -->
    <meta-data
        android:name="channel"
        android:value="android_cn_taptap" />

    <!-- 可选：子渠道 -->
    <meta-data
        android:name="sub_channel"
        android:value="beta" />

    <!-- 可选：其他自定义键 -->
    <meta-data
        android:name="app_id"
        android:value="123456789" />
</application>
```
> Source: [README.md](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/README.md#L112-L128)

### Android 原生実装

插件内置了 Android 原生 JAR 包 (`com.alianhome.getchannel.jar`)，を通じて Android Java 反射呼び出し：

```csharp
// BlankGetChannel.cs 中的 Android 读取逻辑
#if UNITY_ANDROID
using (AndroidJavaClass androidJavaClass = new AndroidJavaClass("com.alianhome.getchannel.MainActivity"))
{
    channelName = androidJavaClass.CallStatic<string>("GetChannel", channelKey);
}
#endif
```
> Source: [BlankGetChannel.cs](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/Runtime/BlankGetChannel.cs#L131-L135)

## Editor/PC/WebGL/UWP/主机平台配置

### 設定ファイル作成

在 Unity プロジェクト的 `Assets/Resources` ファイル夹下作成 `application_config.txt` ファイル：

**ファイル位置：** `Assets/Resources/application_config.txt`

**ファイル格式：**
```
# 每行一个键值对，格式为 key=value
# # 开头为注释

# 必选：主渠道
channel=editor_cn_test

# 可选：子渠道
sub_channel=beta

# 可选：其他自定义键
app_id=123456789
```
> Source: [README.md](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/README.md#L136-L142)

### 读取実装

```csharp
// BlankGetChannel.cs 中的文本配置读取逻辑
var textAsset = Resources.Load<TextAsset>("application_config");
if (textAsset != null)
{
    var lines = textAsset.text.Split(new string[] { "\n", "\r", "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
    foreach (var line in lines)
    {
        var split = line.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries);
        if (split.Length > 1)
        {
            var key = split[0].Trim();
            var channelValue = split[1].Trim();
            ChannelCache[key] = channelValue;
        }
    }
}
```
> Source: [BlankGetChannel.cs](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/Runtime/BlankGetChannel.cs#L106-L121)

### サポート的平台

此配置方法适に使用以下平台：

| 平台 | 宏定义 |
|------|--------|
| Unity Editor | `UNITY_EDITOR` |
| Windows PC | `UNITY_STANDALONE_WIN` |
| macOS PC | `UNITY_STANDALONE_OSX` |
| Linux PC | `UNITY_STANDALONE_LINUX` |
| WebGL | `UNITY_WEBGL` |
| UWP/WSA | `UNITY_WSA` |
| PlayStation 4 | `UNITY_PS4` |
| PlayStation 5 | `UNITY_PS5` |
| Xbox One | `UNITY_XBOXONE` |
| Nintendo Switch | `UNITY_SWITCH` |

## 設定フロー图

```mermaid
flowchart TD
    Start([开始]) --> CheckPlatform{检测当前平台}
    
    CheckPlatform -->|iOS/tvOS/visionOS| iOSFlow[iOS 平台フロー]
    CheckPlatform -->|Android| AndroidFlow[Android 平台フロー]
    CheckPlatform -->|Editor/PC/WebGL/UWP/Console| ConfigFlow[配置ファイルフロー]
    
    iOSFlow --> CheckPlist{Info.plist 中<br/>是否存在 key?}
    CheckPlist -->|是| ReturnExist[返回已有值]
    CheckPlist -->|否| AddDefault[添加默认值<br/>channel=default]
    AddDefault --> ReturnDefault[返回默认值]
    
    AndroidFlow --> ReadMeta[读取 meta-data]
    ReadMeta --> ReturnMeta[返回渠道值]
    
    ConfigFlow --> LoadFile{Resources 加载<br/>application_config.txt}
    LoadFile -->|成功| ParseFile[解析键值对]
    LoadFile -->|失败| ReturnDefault2[返回默认值]
    ParseFile --> ReturnParsed[返回对应值]
    
    ReturnExist --> Cache[存入缓存]
    ReturnDefault --> Cache
    ReturnMeta --> Cache
    ReturnDefault2 --> Cache
    ReturnParsed --> Cache
    
    Cache --> End([结束])
```

## 渠道取得フロー

```mermaid
sequenceDiagram
    participant C as 客户端代码
    participant API as BlankGetChannel
    participant Cache as ChannelCache
    participant Native as 平台原生代码

    C->>API: GetChannelName("channel")
    activate API
    
    API->>Cache: TryGetValue("channel")
    activate Cache
    
    alt 缓存命中
        Cache-->>API: 返回缓存值
        API-->>C: 返回渠道名称
    else 缓存未命中
        Cache-->>API: null
        deactivate Cache
        
        alt UNITY_IOS/TVOS/VISIONOS
            API->>Native: getChannelName("channel")
            Native-->>API: Info.plist 值
        else UNITY_ANDROID
            API->>Native: GetChannel("channel")
            Native-->>API: meta-data 值
        else 其他平台
            API->>API: Resources.Load<TextAsset>
            API-->>API: application_config.txt 内容
        end
        
        API->>Cache: 存入缓存
        activate Cache
        Cache-->>API: 存储完成
        deactivate Cache
        
        API-->>C: 返回渠道名称
    end
    
    deactivate API
```

## 設定注意事项

### 键名一致性

**重要：** 呼び出し `BlankGetChannel.GetChannelName(string key)` 时使用的键名必須与对应平台配置ファイル中設定的键名完全一致。

| 平台 | 配置键名 | 读取方法 |
|------|----------|----------|
| iOS/tvOS/visionOS | 区分大小写 | 直接匹配 |
| Android | 区分大小写 | 直接匹配 |
| 文本ファイル | 区分大小写 | 直接匹配 |

### 默认值处理

当指定键名不存在时，API 会返回默认值：

```csharp
// 默认参数
BlankGetChannel.GetChannelName();  // key="channel", defaultValue="default"

// 指定默认值
BlankGetChannel.GetChannelName("sub_channel", "unknown");
```
> Source: [BlankGetChannel.cs](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/Runtime/BlankGetChannel.cs#L98)

### 缓存机制

插件使用内存缓存避免重复读取配置ファイル：

```csharp
private static readonly Dictionary<string, string> ChannelCache = new Dictionary<string, string>(32);
```
> Source: [BlankGetChannel.cs](https://github.com/GameFrameX/com.gameframex.unity.getchannel/blob/main/Runtime/BlankGetChannel.cs#L57)

**缓存特点：**
- 首次呼び出し时读取配置ファイル
- 后续呼び出し直接从内存返回
- 进程结束时缓存销毁
- 不同 key 分别缓存

## 平台配置快速参考表

| 平台 | 配置ファイル | 键クラス型 | 示例键值 |
|------|-----------|--------|----------|
| iOS/tvOS/visionOS | Info.plist | String | `<key>channel</key><string>ios_cn_taptap</string>` |
| Android | AndroidManifest.xml | meta-data | `android:name="channel" android:value="android_cn_taptap"` |
| 其他平台 | application_config.txt | key=value | `channel=editor_cn_test` |

## よくある質問

### Q1: Android 平台返回 "default" 而非实际配置值

**原因：** Android 原生 JAR 包的 Java クラス路径可能与实际プロジェクト不匹配。

**解決策：**
- 检查 AndroidManifest.xml 中的 `<meta-data>` 标签是否正确配置在 `<application>` 标签内
- 确保 `android:name` プロパティ值与代码中传入的键名一致
- 可在 Android Studio 中检查 Logcat 输出确认呼び出し结果

### Q2: iOS 构建后渠道值为 "unknown"

**原因：** iOS 原生代码读取 Info.plist 失败或键名不匹配。

**解決策：**
- 确认 Info.plist 中存在对应的键名
- 检查键名大小写是否完全匹配
- 确认 Info.plist 已正确打包到 Xcode プロジェクト中

### Q3: Editor 模式下读取不到配置

**原因：** application_config.txt ファイル位置或格式不正确。

**解決策：**
- 确保ファイル位于 `Assets/Resources/application_config.txt`
- 确保ファイル扩展名为 `.txt` 而非 `.json` 或其他
- 检查ファイル编码，建议使用 UTF-8 无 BOM 格式
- ファイル格式必須为标准的 `key=value` 格式

### Q4: Unity 代码裁剪导致取得不到渠道

**原因：** Unity 的 IL2CPP 代码裁剪可能移除了插件関連代码。

**解決策：**
- 插件已包含 `link.xml` ファイル防止代码被裁剪
- 如仍有问题，可在プロジェクト的 `link.xml` 中添加：
```xml
<linker>
    <assembly fullname="BlankGetChannel.Runtime" preserve="all"/>
</linker>
```

## 関連リンク

- [快速开始](./3-getting-started) - 快速集成指南
- [示例](./5-samples) - 代码示例
- [API 参考](./6-api-reference) - 完整 API 文档
- [常见问题](./7-troubleshooting) - 故障排除
- [GitHub 仓库](https://github.com/GameFrameX/com.gameframex.unity.getchannel) - 官方仓库
