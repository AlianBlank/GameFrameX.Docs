...
本文档将帮助您快速上手 GameFrameX Network 网络コンポーネント，了解其コア概念并开始构建您的网络应用程序。

## 概要

GameFrameX Network 是 GameFrameX ゲームフレームワーク的コア网络コンポーネント，提供 TCP 长连接機能，サポート：
- 基づく TCP 的可靠连接
...
sequenceDiagram
    ...
    App->>NC: 关闭连接 (Close)
    NC->>Server: 断开连接
    NC-->>App: 触发 Closed イベント
```

## 定义消息クラス型

首先，必要定义您的お願いします求和响应消息クラス型。消息必須を継承 `MessageObject` クラス：
...
使用 `MessageHandlerAttribute` 特性来注册消息处理函数：
...
```csharp
// 設定 isIgnoreErrorCode = true 时，即使服务器返回错误码也不会抛出異常
...
以下は完整的客户端示例，展示了从连接到处发送消息的完整フロー：
...
public class NetworkExample : MonoBehaviour
{
    ...
    private async void Login(string userName, string password)
    {
        ...
        {
            Debug.LogError($"RPC 呼び出し失败: {ex.Message}");
        }
    }
...
网络コンポーネント使用自定义的二进制协议格式，包含 14 字节的包头：
...
| UniqueId | 4 字节 | 消息唯一编号，に使用 RPC 匹配 |
| MessageId | 4 字节 | 消息クラス型 ID |
...
を通じて `INetworkChannel` インターフェース可能配置以下选项：

| プロパティ | クラス型 | 默认值 | 説明 |
|------|------|--------|------|
| HeartBeatInterval | float | 30f | 心跳发送间隔（秒） |
| ResetHeartBeatElapseSecondsWhenReceivePacket | bool | true | 收到消息时是否重置心跳计时 |
| PacketSendHeaderHandler | IPacketSendHeaderHandler | DefaultPacketSendHeaderHandler | 发送包头处理器 |
| PacketSendBodyHandler | IPacketSendBodyHandler | - | 发送包体处理器 |
| PacketReceiveHeaderHandler | IPacketReceiveHeaderHandler | DefaultPacketReceiveHeaderHandler | 接收包头处理器 |
| PacketReceiveBodyHandler | IPacketReceiveBodyHandler | - | 接收包体处理器 |
| MessageCompressHandler | IMessageCompressHandler | - | 消息压缩处理器 |
| MessageDecompressHandler | IMessageDecompressHandler | - | 消息解压处理器 |
...
- [概述](./1-overview) - 了解更多关于网络コンポーネント的整体设计
- [安装](./2-installation) - 安装和配置网络コンポーネント
- [网络管理器](./4-core-concepts.1-network-manager) - NetworkManager 详细 API
- [网络频道](./4-core-concepts.2-network-channel) - NetworkChannel 详细 API
- [消息クラス型](./4-core-concepts.3-message-types) - 消息クラス型系统详解
- [数据包处理器](./4-core-concepts.4-packet-handlers) - 数据包处理机制
- [心跳机制](./5-heartbeat) - 心跳检测机制详解
- [イベント系统](./6-events) - 网络イベント系统
- [消息压缩](./7-message-compression) - 消息压缩配置
- [RPC 远程呼び出し](./8-rpc) - RPC 呼び出し机制
- [常见问题](./10-troubleshooting) - 常见问题与解決策
