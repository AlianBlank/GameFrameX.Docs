...
PlaySoundParams クラスに使用配置サウンド播放的各种プロパティ，包括音量、音调、循环、淡入淡出等参数。这些参数在播放サウンド时传递给 SoundManager，以控制サウンド的具体播放方法。

## 概要

PlaySoundParams 是サウンド系统的コア参数クラス，它封装了播放サウンド时所需的所有配置选项。该クラス実装了 IReference インターフェース，サポートオブジェクトプール化管理，可能重复使用以减少内存分配。

在呼び出し ISoundManager.PlaySound メソッド时，可能传入 PlaySoundParams インスタンス来定制化サウンド的播放行为。如果传入 null 或不传入参数，系统将使用默认参数值。

### 设计意图

PlaySoundParams 采用参数对象模式（Parameter Object Pattern），将多个関連的播放参数封装为一个对象。这种设计：

- **简化 API**：避免在 PlaySound メソッド中传递大量独立参数
- **提高可读性**：を通じて命名プロパティ使参数含义清晰明了
- **サポート默认值**：所有参数都有合理的默认值，无需全部設定
- **オブジェクトプール化**：実装 IReference インターフェース，サポート ReferencePool 回收复用
...
| 参数 | クラス型 | 默认值 | 説明 |
|------|------|--------|------|
| MuteInSoundGroup | bool | false | 在所属サウンド组内是否静音 |
| VolumeInSoundGroup | float | 1f | 在所属サウンド组内的音量（0.0-1.0） |
| Priority | int | 0 | サウンド优先级，数值越高优先级越高 |
...
| 参数 | クラス型 | 默认值 | 説明 |
|------|------|--------|------|
| SpatialBlend | float | 0f | 空间混合量，0 为 2D サウンド，1 为 3D サウンド |
| MaxDistance | float | 100f | 最大衰减距离（仅 3D サウンド有效） |
...
設定サウンド开始播放的位置（以秒为单位）。这对于実装断点续播、预览機能或跳过片头非常有用。
...
控制サウンド是否循环播放。对于背景音乐或环境音效通常設定为 true，对于一次性音效設定为 false。
...
設定サウンド开始播放时的淡入时间，使音量从 0 逐渐过渡到目标音量。这对于避免突然的サウンド爆发非常有用。
...
控制该サウンド在其所属サウンド组内是否静音。注意：即使設定为静音，该サウンド仍然会占用サウンド组的播放通道。
...
設定该サウンド在其所属サウンド组内的音量系数。最终音量 = サウンド组音量 × 个体音量。
...
設定サウンド的优先级。当サウンド组中没有可用的播放通道时，低优先级的サウンド会被高优先级的替换。优先级范围建议：0-255，数值越高优先级越高。
...
调整サウンド的播放速度/音调。1.0 为正常速度，小于 1.0 为慢速/低音调，大于 1.0 为快速/高音调。
...
控制サウンド在左右声道之间的位置。-1.0 为完全左声道，0.0 为居中，1.0 为完全右声道。
...
控制サウンド从 2D（立体声）到 3D（空间化）的过渡。0.0 为完全 2D サウンド，1.0 为完全 3D サウンド。

```csharp
var playSoundParams = PlaySoundParams.Create();
playSoundParams.SpatialBlend = 1.0f; // 完全 3D サウンド
...
設定 3D サウンド的最大衰减距离。超过此距离后，サウンド音量不再继续衰减。
...
設定 3D サウンド的多普勒效应强度。当声源和听者相对移动时，会产生多普勒频移效果。0.0 为禁用，1.0 为正常强度。
...
推荐使用静态工厂メソッド `Create()` 作成 PlaySoundParams インスタンス，该メソッド从オブジェクトプール中取得インスタンス：
...
PlaySoundParams 中的参数最终会映射到 Unity 的 AudioSource コンポーネント：
...
1. **オブジェクトプール管理**：使用 `PlaySoundParams.Create()` 作成的参数在播放完成后会自动释放回オブジェクトプール，无需手动释放。

2. **如果参数为 null**：呼び出し PlaySound 时传入 null 参数，系统会自动作成默认参数的 PlaySoundParams。

3. **优先级与通道**：当サウンド组的所有通道都被占用时，低优先级的正在播放的サウンド会被新来的高优先级サウンド替换。

4. **音量叠加**：最终音量 = サウンド组音量（SoundGroup.Volume）× 个体音量（VolumeInSoundGroup）。

5. **空间音频**：空间関連参数（MaxDistance、DopplerLevel）必要将 SpatialBlend 設定为大于 0 的值才能生效。

## 関連リンク

- [インターフェース定义](./5-api-reference.1-interfaces) - ISoundManager 和関連インターフェース
- [イベント参数](./5-api-reference.3-event-args) - 播放イベント関連参数
- [サウンド管理器](./4-core-concepts.1-sound-manager) - サウンド管理器详解
