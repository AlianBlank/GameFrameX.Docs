...
界面组（UI Group）是 GameFrameX フレームワーク中に使用管理 UI 界面层级和显示顺序的コア机制。FairyGUI 插件提供します `FairyGUIUIGroupHelper` クラス来実装符合 FairyGUI 渲染特性的界面组機能。

## 概要

界面组配置主要に使用：

- **层级管理**：控制不同クラス型界面的显示层级（如弹窗、对话框、主界面等）
- **深度控制**：を通じて設定 Z 轴位置来决定界面的前后顺序
- **容器组织**：将関連界面组织在同一个容器中，便于管理和控制

在 FairyGUI 集成中，界面组を通じて GComponent 容器実装，每个组对应一个独立的容器，界面作为子元素添加到对应的组容器中。
...
flowchart TD
    ...
    subgraph FairyGUI_Runtime["FairyGUI 运行时"]
        GRoot[GRoot<br/>根容器"]
        GComponent["GComponent<br/>容器コンポーネント"]
    end
...
4. **FairyGUIFormHelper**：界面辅助器，负责将界面インスタンス添加到对应的界面组容器中
...
sequenceDiagram
    ...
    UC->>UG: 作成界面组 (name, depth)
    UG->>UGH: Handler(root, groupName, typeName, helper, depth)
    UGH->>GC: 作成新 GComponent
    GC->>GC: 設定容器プロパティ (name, opaque)
...
sequenceDiagram
    ...
    alt UIGroup 为空
        ...
        UG-->>FH: 返回对应界面组
        FH->>UF: 設定 UIGroup プロパティ
    end
    
    FH->>UG: 取得 Helper (MonoBehaviour)
    UG->>GC: 取得 DisplayObjectInfo
    GC->>GC: 取得 GOwner as GComponent
...
| プロパティ | クラス型 | 説明 |
|------|------|------|
| `Depth` | int | 取得或設定界面组深度，に使用控制 Z 轴顺序 |

### FairyGUIUIGroupHelper メソッド

#### `SetDepth(int depth)`

設定界面组深度。该メソッドを通じて修改 transform.localPosition 的 Z 坐标来実装深度控制。
...
を通じて `OptionUIGroupAttribute` プロパティ标记，将界面分配到指定的界面组：
...
**説明：**
- 界面组名称在 GameFrameX コアフレームワーク的 UIComponent 中配置
- FairyGUIUIGroupHelper 会自动作成对应名称的 GComponent 容器
- 深度值决定了界面的显示顺序，数值越大越靠前

### 設定界面组深度

界面组的深度通常在ゲームフレームワーク初期化时配置：

```csharp
csharp
// 在 UIComponent 配置中設定界面组深度
// 不同界面组可能使用不同的深度值来実装层级分离
...
将所有弹窗クラス界面放在独立的界面组中，确保弹窗始终显示在普通界面之上：
...
- [API 参考 - FairyGUIUIGroupHelper クラス](./5-api-reference.3-form-helper)
