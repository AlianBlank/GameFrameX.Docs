...
本页收录 com.gameframex.unity.payment 支付模块的全部版本变更内容,原文取自仓库根目录的 [CHANGELOG.md](https://github.com/GameFrameX/com.gameframex.unity.payment/blob/main/CHANGELOG.md),不做扩写与逐条解读。当前包版本为 **1.1.0**(最低 Unity 版本 2019.4,依赖 `com.gameframex.unity` 2.5.1 与 `com.gameframex.unity.payment.google` 1.0.1,见 [package.json](https://github.com/GameFrameX/com.gameframex.unity.payment/blob/main/package.json#L6-L27))。

## 版本概览

| 版本 | 发布日期 | 主要内容 |
|------|----------|----------|
| 1.1.0 | 2026-04-21 | 新增 PurchaseParams 购买参数基类、支付事件回调系统、WebGL/PC/macOS 平台配置;修复 DefaultPaymentManager 多处缺陷 |
| 1.0.0 | 2025-12-23 | 支付模块基础实现 |
...
| 升级路径 | 需要注意的变化 |
|----------|----------------|
| 1.0.0 → 1.1.0 | 旧的 `Buy()` 方法已被标记过期,新代码请改用基于 `PurchaseParams` 的购买方法 |
| 1.0.0 → 1.1.0 | macOS 从 PC 平台支付配置中独立出来,若你的项目面向 macOS,需检查平台配置是否仍然生效 |
| 1.0.0 → 1.1.0 | `DefaultPaymentManager.IsReady()` 现在返回 `false`,且所有方法会输出未实现警告日志——依赖默认实现的项目需接入具体支付平台组件 |
| 任意版本 | `link.xml` 防裁剪方案已回滚,改为属性标记;若你自定义过裁剪配置,请确认 CroppingHelper 覆盖的新类型未被裁剪 |

## 相关链接

- [CHANGELOG.md](https://github.com/GameFrameX/com.gameframex.unity.payment/blob/main/CHANGELOG.md) — 变更记录源文件
- [package.json](https://github.com/GameFrameX/com.gameframex.unity.payment/blob/main/package.json) — 当前版本号与依赖声明
- 版本对比:[1.0.0...1.1.0](https://github.com/gameframex/com.gameframex.unity.payment/compare/1.0.0...1.1.0)
