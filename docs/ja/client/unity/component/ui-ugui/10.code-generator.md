...
代码生成器是 GameFrameX UGUI 插件的コア编辑器工具，能够自动从 Unity UGUI 预制体生成对应的 C# 代码，大幅提升 UI 开发效率。

## 概要

代码生成器（UGUICodeGenerator）は Unity 编辑器工具，に使用自动将 UGUI 预制体转换为可用的 C# 代码。它能够：

- **自动遍历 UI 层级**：递归扫描预制体中的所有子对象
- **智能クラス型识别**：自动识别 Button、Text、Image、Toggle 等 UI コンポーネントタイプ
- **可扩展的转换机制**：を通じてインターフェースサポート自定义 UI コンポーネントタイプ识别
- **一键代码生成**：从菜单或检查器触发代码生成

### 使用シーン

当必要作成新的 UI 界面时，无需手动编写大量重复的 UI 引用代码，只需：
1. 在 Unity 编辑器中作成或选择 UGUI 预制体
2. 右键选择生成代码菜单
3. 系统自动生成包含所有 UI 引用的代码ファイル

## 架构

代码生成器采用モジュラー設計，主要包含以下コンポーネント：

```mermaid
graph TD
    subgraph Editor["编辑器层"]
        Menu[菜单项<br/>GameObject/UI/Generate UGUI Code]
        Generator[UGUICodeGenerator<br/>代码生成器コア]
        Inspector[UGUIComponentInspector<br/>コンポーネント检查器]
    end

    subgraph Core["コア处理层"]
        Handler[クラス型转换处理器<br/>IUGUIGeneratorCodeConvertTypeHandler]
        NodeInfo[NodeInfo<br/>节点信息]
        Convert[ConvertType<br/>クラス型转换]
    end

    subgraph Runtime["运行时层"]
        UGUI[UGUI<br/>基クラス]
        Attribute[UGUIElementPropertyAttribute<br/>プロパティ标记]
    end
...
| IUGUIGeneratorCodeConvertTypeHandler | `Editor/IUGUIGeneratorCodeConvertTypeHandler.cs` | クラス型转换处理器インターフェース，サポート扩展 |
| UGUIComponentInspector | `Editor/UGUIComponentInspector.cs` | Unity 检查器，提供重新生成和绑定機能 |
...
代码生成器按照优先级依次检测以下 UI コンポーネントタイプ：
...
- GameFrameX.UI.UGUI.Runtime.UIImage（扩展コンポーネント）
- TMPro.TextMeshProUGUI（条件编译）

### クラス型转换处理器扩展

を通じて実装 `IUGUIGeneratorCodeConvertTypeHandler` インターフェース，可能添加自定义的 UI コンポーネントタイプ识别逻辑：
...
处理器按优先级排序后依次执行，一旦某个处理器返回非空クラス型，即采用该クラス型：
...
1. **选择 UGUI 预制体**：在 Unity 编辑器 Hierarchy 或 Project 窗口中选择必要生成代码的 UGUI 预制体

2. **触发代码生成**：
   - 方法一：右键点击预制体 → `GameObject/UI/Generate UGUI Code(生成UGUI代码)`
   - 方法二：在预制体的 Inspector 中点击"重新生成代码"按钮

3. **查看生成的代码**：代码会自动生成到指定ディレクトリ
...
在预制体的 Inspector 面板中，可能点击"重新绑定列表"按钮来重新绑定所有 UI 引用：
...
代码生成器に基づいて预制体位置自动选择生成路径：

| 预制体位置 | 生成路径 |
|------------|----------|
| 包含 "Resources" ディレクトリ | `Assets/Scripts/Game/UGUI/{预制体名称}/` |
...
可能を通じて実装 `IUGUIGeneratorCodeConvertTypeHandler` インターフェース来添加自定义的 UI コンポーネント识别逻辑：
