...
This document will help you快速上手 GameFrameX Network Network Component，了解其Core Concepts并开始构建您的网络Application。

## 概述

GameFrameX Network 是 GameFrameX 游戏框架的核心Network Component，provides TCP 长connect功能，supports：
- 基于 TCP 的可靠connect
- Protocol Buffers 消息serialize
- RPC 远程过程调用
- Heartbeat Check与自动重连
- Message Compression与decompress
...
首先，需要定义您的请求和响应Message Types。消息必须Inherits自 `MessageObject` 类：
...
## CreatesMessage Handlers

Uses `MessageHandlerAttribute` 特性来register消息process函数：
...
## InitializeNetwork Component

### 步骤 1：InitializeMessage Types

在Applicationstart时，需要InitializeMessage Typesmap：
...
以下是一个完整的Client示例，展示了从connect到处Send Message的完整流程：
...
Network ComponentUses自定义的二进制协议格式，contains 14 字节的包头：
...
|------|------|------|
| PacketLength | 4 字节 | 整个数据包的长度（网络字节序） |
| OperationType | 1 字节 | 操作Type：1=心跳，4=普通消息 |
| ZipFlag | 1 字节 | compress标志：0=未compress，1=已compress |
| UniqueId | 4 字节 | Message unique ID，used for RPC 匹配 |
| MessageId | 4 字节 | Message Types ID |
...
through `INetworkChannel` 接口可以configuration以下选项：

| Property | Type | Default | Description |
|------|------|--------|------|
| HeartBeatInterval | float | 30f | 心跳send间隔（秒） |
| ResetHeartBeatElapseSecondsWhenReceivePacket | bool | true | 收到消息时是否reset心跳计时 |
| PacketSendHeaderHandler | IPacketSendHeaderHandler | DefaultPacketSendHeaderHandler | send包头process器 |
| PacketSendBodyHandler | IPacketSendBodyHandler | - | send包体process器 |
| PacketReceiveHeaderHandler | IPacketReceiveHeaderHandler | DefaultPacketReceiveHeaderHandler | receive包头process器 |
| PacketReceiveBodyHandler | IPacketReceiveBodyHandler | - | receive包体process器 |
| MessageCompressHandler | IMessageCompressHandler | - | Message Compressionprocess器 |
| MessageDecompressHandler | IMessageDecompressHandler | - | 消息decompressprocess器 |
...
- [概述](./1-overview) - Learn more aboutNetwork Component的整体设计
- [installation](./2-installation) - installation和configurationNetwork Component
- [网络Manages器](./4-core-concepts.1-network-manager) - NetworkManager 详细 API
- [Network Channel](./4-core-concepts.2-network-channel) - NetworkChannel 详细 API
- [Message Types](./4-core-concepts.3-message-types) - Message Types系统详解
- [数据包process器](./4-core-concepts.4-packet-handlers) - 数据包process机制
- [Heartbeat Mechanism](./5-heartbeat) - Heartbeat Check机制详解
- [Event System](./6-events) - 网络Event System
- [Message Compression](./7-message-compression) - Message Compressionconfiguration
- [RPC Remote Procedure Call](./8-rpc) - RPC 调用机制
- [Troubleshooting](./10-troubleshooting) - Troubleshooting与解决方案
