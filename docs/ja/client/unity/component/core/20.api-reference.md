...
本文档提供 GameFrameX フレームワーク所有公共 API 的完整参考，包括コアクラス、インターフェース、メソッド签名和使用説明。

## 概要

GameFrameX フレームワーク采用模块化アーキテクチャ設計，コア API 主要分为以下几个部分：

- **フレームワークエントリーポイント**：`GameFrameworkEntry` 和 `GameEntry` 提供フレームワーク初期化和コンポーネント管理
- **基础コンポーネント**：`BaseComponent` 提供ゲーム运行时基础設定
- **参照プール**：`ReferencePool` 提供高性能内存管理
- **イベント系统**：`EventPool<T>` 提供クラス型安全的イベント机制
- **任务池**：`TaskPool<T>` 提供异步任务管理
- **オブジェクトプール**：`ObjectPoolComponent` 提供ゲーム对象复用
- **工具クラス**：`Utility` 名前空間提供各クラス辅助機能
...
フレームワーク模块管理器，负责フレームワーク级模块的注册、取得和更新。
...
关闭并清理所有ゲームフレームワーク模块，释放参照プール和缓存。
...
取得指定インターフェースクラス型的フレームワーク模块。如果模块不存在，将自动作成。
...
Unity コンポーネント管理器，提供ゲームフレームワークコンポーネント的注册和取得。
...
ゲーム基础コンポーネント，提供帧率控制、ゲーム速度管理、后台运行設定等コア機能。
...
| `GameSpeed` | `float` | 取得或設定ゲーム速度（0 表示暂停） |
| `IsGamePaused` | `bool` | 取得ゲーム是否暂停 |
...
| プロパティ | クラス型 | 説明 |
|------|------|------|
| `EnableStrictCheck` | `bool` | 取得或設定是否开启强制クラス型检查 |
| `Count` | `int` | 取得参照プール的数量 |
...
引用インターフェース，所有可放入参照プール的对象必須実装此インターフェース。
...
| プロパティ | クラス型 | 説明 |
|------|------|------|
| `EventHandlerCount` | `int` | 取得イベント处理函数的数量 |
| `EventCount` | `int` | 取得队列中待处理イベント的数量 |
...
イベント参数基クラス，所有自定义イベント参数都应继承此クラス。
...
Unity オブジェクトプールコンポーネント，提供ゲーム对象的复用管理。
...
| プロパティ | クラス型 | 説明 |
|------|------|------|
| `Count` | `int` | 取得オブジェクトプール数量 |

#### コアメソッド

##### `HasObjectPool<T>(): bool`

检查是否存在指定クラス型的オブジェクトプール。
...
| `Fatal(message)` | 致命错误日志 |

```csharp
Log.Info("玩家 {0} 进入了ゲーム", playerName);
Log.Warning("リソース加载失败: {0}", url);
Log.Error("网络お願いします求超时");
```
...
- [运行时模块 - イベント系统](./5-runtime.event-system)
- [运行时模块 - オブジェクトプールシステム](./5-runtime.object-pool)
- [运行时模块 - 参照プールシステム](./5-runtime.reference-pool)
- [运行时模块 - 任务池系统](./5-runtime.task-pool)
- [运行时模块 - 工具クラス](./5-runtime.utility-classes)
