...
GameFrameX フレームワーク提供的コア静态工具函数库，涵盖加密解密、哈希计算、JSONシリアライズ、压缩解压、字符串処理、随机数生成、数据校验、网络操作、ID生成等常用機能。

## 概要

Utility 工具类是 GameFrameX フレームワーク的基本设施层，为上层业务逻辑提供底层技术支撑。这些工具类采用**静态类設計**，无需インスタンス化つまり可直接呼び出し，极大地簡素化了日常开发工作。

### 設計理念

1. **无ステータス工具メソッド**：所有メソッド都是无副作用的静态メソッド，可安全并行呼び出し
2. **可拡張アーキテクチャ**：关键機能采用 Helper 插拔机制，サポートユーザー自定義実装
3. **Partial Class 组织**：使用 C# partial class 特性，按機能モジュール分散コード到多个ファイル，提高可维护性
...
|------|----------|
| 加密モジュール | XOR 异或加密 |
| 哈希モジュール | MD5、SHA1、SHA256、HMAC-SHA256、MurmurHash3、XxHash |
| 数据フォーマット | JSON シリアライズ/反シリアライズ、压缩/解压缩 |
| 文本処理 | 字符串フォーマット化、タイプ转换、拡張字符串 |
| 校验モジュール | CRC32、CRC64 校验和计算 |
| 随机数 | 随机整数、浮点数、字节数组生成 |
...
```mermaid
graph TD
    subgraph Client["业务层调用"]
        APP["业务代码"]
    end
    
    subgraph Utility["Utility 工具类"]
        MAIN["Utility 主类<br/>(partial)"]
...
提供 XOR 异或加密機能，適しています轻量级数据混淆シーン。
...
サポート多种哈希算法，用于数据完全な性校验和密码存储：

- **MD5**：128位哈希值，適していますファイル完全な性校验
- **SHA1**：160位哈希值
- **SHA256**：256位哈希值，安全系数更高
- **HMAC-SHA256**：带密钥的哈希，用于メッセージ认证
...
提供 JSON シリアライズ和反シリアライズ機能，サポート可插拔的 Helper 実装：
...
提供安全的字符串フォーマット化機能，サポート最多16个パラメータ的フォーマット化：
...
提供 CRC32/CRC64 校验和计算，用于数据完全な性検証：
...
以下例表示如何在实际プロジェクト中使用 Utility 工具类：

```csharp
using GameFrameX.Runtime;

// 1. 初始化 Helper（可选，如需自定义实现）
Utility.Json.SetJsonHelper(new NewtonsoftJsonHelper());
Utility.Compression.SetCompressionHelper(new DefaultCompressionHelper());
Utility.Text.SetTextHelper(new DefaultTextHelper());
...
- [辅助类](./5-runtime.helper-classes) - Unity 特定シーン的辅助コンポーネント
- [拡張メソッド库](./5-runtime.extension-methods) - 链式呼び出し拡張メソッド
- [イベント系统](./5-runtime.event-system) - イベント驱动アーキテクチャ
- [对象池系统](./5-runtime.object-pool) - 对象复用管理
