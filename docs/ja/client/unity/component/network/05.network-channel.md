...
网络频道（Network Channel）是 GameFrameX 网络フレームワーク中的コアコンポーネント，负责管理与远程服务器的单一连接。它封装了 Socket 通信、消息序列化/反序列化、心跳检测、RPC 呼び出し等完整機能，使開発者能够方便地进行网络通信开发。

## 概要

在 GameFrameX 网络フレームワーク中，每个网络频道代表一个独立的网络连接。フレームワークサポート作成多个频道同时连接不同的服务器，每个频道维护自己的连接ステータス、发送队列、接收队列和心跳ステータス。

网络频道的设计采用了分层架构：

- **インターフェース层**：定义 `INetworkChannel` インターフェース，抽象所有网络操作
- **基クラス层**：提供 `NetworkChannelBase` 抽象基底クラス，実装通用逻辑
- **実装层**：包括 `SystemTcpNetworkChannel`（TCP 连接）和 `WebSocketNetworkChannel`（WebSocket 连接）
- **辅助层**：を通じて `INetworkChannelHelper` 委托消息的序列化和反序列化
...
4. **心跳机制**：定期发送心跳包检测连接ステータス
5. **RPC 呼び出し**：サポートお願いします求-响应模式的远程过程呼び出し
6. **消息压缩**：サポート消息压缩以减少网络传输量
7. **イベント通知**：を通じてイベント系统通知连接ステータス变更
...
graph TD
    ...
    subgraph "网络频道层"
        ...
        BaseChannel[NetworkChannelBase 基クラス]
...
| コンポーネント | 説明 |
|------|------|
| `INetworkChannel` | 网络频道インターフェース，定义所有网络操作 |
| `NetworkChannelBase` | 抽象基底クラス，実装通用逻辑和ステータス管理 |
| `SystemTcpNetworkChannel` | 基づく System.Net.Sockets 的 TCP 実装 |
| `WebSocketNetworkChannel` | 基づく WebSocket 的连接実装 |
| `INetworkChannelHelper` | 消息序列化/反序列化辅助器インターフェース |
| `DefaultNetworkChannelHelper` | 默认実装，自动注册处理器 |

## 消息处理フロー

### 发送フロー

当客户端呼び出し `Send` メソッド发送消息时，网络频道按照以下フロー处理：
...
sequenceDiagram
    ...
    Note over Helper: 解析包头取得消息长度和ID
...
网络频道内置心跳机制，に使用检测连接是否仍然有效。

```mermaid
flowchart TD
    Start([Update 轮询]) --> CheckActive{频道活跃?}
    CheckActive -->|否| End([结束])
    CheckActive -->|是| UpdateTimer[更新心跳タイマー]
    UpdateTimer --> CheckTimeout{タイマー >= 心跳间隔?}
    CheckTimeout -->|否| End
    CheckTimeout -->|是| SendHeartBeat[发送心跳包]
    SendHeartBeat --> ResetTimer[重置タイマー]
    ResetTimer --> CheckMissCount{丢失次数 > 最大允许?}
    CheckMissCount -->|是| CloseChannel[Close 关闭连接]
    CloseChannel --> FireEvent[触发 NetworkMissHeartBeat イベント]
    FireEvent --> End
...
| `ResetHeartBeatElapseSecondsWhenReceivePacket` | false | 收到消息时是否重置心跳タイマー |
...
// 開発者可能を通じて继承 INetworkChannelChannelHelper 自定义处理器注册逻辑
...
| `Call<TResult>(MessageObject messageObject, bool isIgnoreErrorCode = false)` | 发送 RPC お願いします求并等待响应 |
...
| `AddressFamily` | AddressFamily | 取得地址クラス型（IPv4/IPv6） |
| `Socket` | INetworkSocket | 取得底层 Socket |
| `SendPacketCount` | int | 取得待发送消息数量 |
| `SentPacketCount` | int | 取得已发送消息总数 |
| `ReceivedPacketCount` | int | 取得已接收消息总数 |
...
| メソッド | 説明 |
|------|------|
| `SetRPCErrorCodeHandler(EventHandler<MessageObject>)` | 設定 RPC 错误码回调 |
| `SetRPCErrorHandler(EventHandler<MessageObject>)` | 設定 RPC 错误回调 |
| `SetRPCStartHandler(EventHandler<MessageObject>)` | 設定 RPC 开始回调 |
| `SetRPCEndHandler(EventHandler<MessageObject>)` | 設定 RPC 结束回调 |
...
## INetworkChannelHelper インターフェース参考

`INetworkChannelHelper` 是网络频道的辅助器インターフェース，负责消息的序列化和反序列化。
