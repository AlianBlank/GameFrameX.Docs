# 脚本宏定义

脚本宏定义（Scripting Define Symbols）是 Unity 项目中用于条件编译的预处理器符号。GameFrameX Config 组件通过脚本宏定义来控制二进制配置表功能的开启与关闭。

## 概述

脚本宏定义是 Unity 编译器的一个特性，允许开发者在代码中根据不同的编译条件包含或排除特定代码段。GameFrameX Config 组件使用 `ENABLE_BINARY_CONFIG` 脚本宏来控制是否启用二进制配置表功能。

### 什么是二进制配置表

二进制配置表是一种以二进制格式存储配置数据的方式，相比文本格式（CSV、TSV）具有以下优势：

- **加载速度更快**：二进制数据无需解析，可直接读取
- **文件体积更小**：数据以紧凑的二进制格式存储
- **安全性更高**：二进制数据不易被直接阅读和修改
- **解析开销更低**：无需字符串分割和类型转换

## 工作原理

### 架构流程

```mermaid
flowchart TD
    subgraph UnityEditor["Unity 编辑器"]
        Menu1["菜单项：开启二进制配置表"]
        Menu2["菜单项：关闭二进制配置表"]
        SymbolMgr["ScriptingDefineSymbols 工具类"]
        PlayerSettings["PlayerSettings"]
    end

    subgraph ProjectBuild["项目构建"]
        Preprocessor["预处理器"]
        Compile["条件编译"]
    end

    subgraph Runtime["运行时"]
        ConfigSys["配置系统"]
        BinaryParser["二进制解析器"]
        TextParser["文本解析器"]
    end

    Menu1 -->|EnableBinaryConfig| SymbolMgr
    Menu2 -->|DisableBinaryConfig| SymbolMgr
    SymbolMgr -->|修改符号| PlayerSettings
    PlayerSettings -->|包含符号| Preprocessor
    Preprocessor -->|编译代码| Compile
    Compile -->|启用| BinaryParser
    Compile -->|禁用| BinaryParser
    ConfigSys --> BinaryParser
    ConfigSys --> TextParser
```

### 符号作用机制

```mermaid
flowchart LR
    subgraph 定义["符号定义阶段"]
        D1[Editor 菜单设置] --> D2[写入 PlayerSettings]
    end

    subgraph 编译["编译阶段"]
        D2 --> C1[Unity 预处理]
        C1 --> C2{符号是否存在?}
        C2 -->|是| C3[包含二进制配置代码]
        C2 -->|否| C4[排除二进制配置代码]
    end

    subgraph 运行["运行阶段"]
        C3 --> R1[加载 .bytes 文件]
        C4 --> R2[加载 .txt/.csv 文件]
    end
```

## 使用方法

### 开启二进制配置表

在 Unity 编辑器中，通过菜单操作开启二进制配置功能：

1. 打开 Unity 编辑器
2. 依次点击菜单：`GameFrameX` → `Scripting Define Symbols` → `Enable Binary Config(开启二进制配置表)`
3. 等待 Unity 重新编译脚本

### 关闭二进制配置表

如需切换回文本格式配置：

1. 依次点击菜单：`GameFrameX` → `Scripting Define Symbols` → `Disable Binary Config(关闭二进制配置表)`
2. 等待 Unity 重新编译脚本

## API 参考

### ConfigDefineSymbols

命名空间：`GameFrameX.Config.Editor`

```csharp
public static class ConfigDefineSymbols
```

配置表二进制功能的脚本宏定义类，提供二进制配置表开关功能。

#### 字段

| 字段 | 类型 | 描述 |
|------|------|------|
| `EnableBinaryConfigScriptingDefineSymbol` | `const string` | 二进制配置表脚本宏定义的常量值：`"ENABLE_BINARY_CONFIG"` |

#### 方法

##### EnableBinaryConfig

```csharp
[MenuItem("GameFrameX/Scripting Define Symbols/Enable Binary Config(开启二进制配置表)", false, 501)]
public static void EnableBinaryConfig()
```

开启二进制配置表功能的脚本宏定义。

**功能说明：**
将 `ENABLE_BINARY_CONFIG` 符号添加到项目的 PlayerSettings 中，使项目在编译时包含二进制配置相关的代码。

> Source: [ConfigDefineSymbols.cs](https://github.com/GameFrameX/com.gameframex.unity.config/blob/main/Editor/ConfigDefineSymbols.cs#L25-L29)

---

##### DisableBinaryConfig

```csharp
[MenuItem("GameFrameX/Scripting Define Symbols/Disable Binary Config(关闭二进制配置表)", false, 500)]
public static void DisableBinaryConfig()
```

关闭二进制配置表功能的脚本宏定义。

**功能说明：**
从项目的 PlayerSettings 中移除 `ENABLE_BINARY_CONFIG` 符号，使项目在编译时不包含二进制配置相关的代码。

> Source: [ConfigDefineSymbols.cs](https://github.com/GameFrameX/com.gameframex.unity.config/blob/main/Editor/ConfigDefineSymbols.cs#L16-L20)

## 配置选项

| 配置项 | 类型 | 默认值 | 描述 |
|--------|------|--------|------|
| `ENABLE_BINARY_CONFIG` | 脚本宏 | 未定义 | 控制二进制配置表功能的开关 |

## 注意事项

1. **编译重载**：修改脚本宏定义后，Unity 会自动重新编译脚本，此时可能短暂卡顿，属正常现象。

2. **平台差异**：脚本宏定义是全局的，对所有平台生效。如需针对特定平台设置，请手动在 PlayerSettings 中修改。

3. **资源格式**：开启二进制配置后，需要确保配置资源文件为 `.bytes` 格式。

4. **数据兼容性**：二进制格式和文本格式的数据结构可能不同，切换时需确保数据已正确转换。

## 相关链接

- [二进制配置表](./6-configuration.binary-config)
- [ConfigComponent 配置组件](./4-core-concepts.config-component)
- [ConfigManager 配置管理器](./4-core-concepts.config-manager)
- [IDataTable 数据表接口](./4-core-concepts.idata-table)
