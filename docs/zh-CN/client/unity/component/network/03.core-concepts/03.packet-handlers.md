...
数据包处理器是 GameFrameX 网络库的核心组件，负责网络消息的序列化和反序列化操作。它们将应用层的消息对象转换为网络字节流（发送时），并将接收到的字节流转换回消息对象（接收时）。

## 概述

在网络通信中，数据需要经过特定的格式处理才能在网络中传输。数据包处理器正是承担这一职责的关键组件，它们定义了网络消息的结构和解析方式，使得开发者可以专注于业务逻辑而无需关心底层网络通信细节。

GameFrameX 网络库提供了五类数据包处理器：
- **发送包头处理器**：负责序列化消息头部信息
- **发送包体处理器**：负责序列化消息体内容
...
上图展示了数据包处理器在网络通信中的核心位置。它们通过网络辅助器（INetworkChannelHelper）被调用，完成消息的序列化和反序列化工作。

## 消息发送流程

当客户端需要发送消息时，数据包处理器按照特定的顺序处理消息对象：
...
发送流程分为两个主要阶段：
1. **包头序列化**：发送包头处理器首先被调用，它获取消息的类型信息（消息ID），将消息对象序列化为字节数组，根据配置决定是否压缩，然后写入包头信息（包长度、操作类型、压缩标记、唯一ID、消息ID）
...
接收流程同样分为两个阶段：
1. **包头反序列化**：接收包头处理器从字节流中解析出包长度、操作类型、压缩标记、唯一ID和消息ID等信息
2. **包体反序列化**：接收包体处理器根据消息ID找到对应的消息类型，使用 ProtoBuf 反序列化为消息对象

## 接口定义

### IPacketHandler

标记接口，所有数据包处理器都需要实现此接口：
...
如果默认的处理器无法满足需求，开发者可以创建自定义的数据包处理器。自定义处理器需要实现相应的接口并继承 `IPacketHandler` 标记接口。
