# 添加定时任务

本文档介绍如何在 GameFrameX Timer 组件中添加各类定时任务。

## 概述

Timer 组件提供了三种添加定时任务的方式：

1. **Add 方法** - 添加可重复执行的定时任务
2. **AddOnce 方法** - 添加仅执行一次的任务
3. **AddUpdate 方法** - 添加每帧执行的任务

所有任务都通过 TimerComponent 暴露给 Unity 组件使用，内部由 TimerManager 实现具体的定时逻辑。

## 核心概念

### 定时任务参数

| 参数 | 类型 | 说明 |
|------|------|------|
| interval | float | 间隔时间（秒） |
| repeat | int | 重复次数（0 表示无限重复） |
| callback | Action\<object\> | 回调函数 |
| callbackParam | object | 传递给回调的参数（可选） |

### 任务状态管理

TimerManager 使用以下数据结构管理任务：

- `_items` - 正在执行的任务字典
- `_toAdd` - 等待添加到执行队列的任务
- `_toRemove` - 等待移除的任务
- `_pool` - 对象池，用于复用 TimerItem 对象

```mermaid
flowchart TD
    subgraph Client["客户端代码"]
        TC[TimerComponent]
    end

    subgraph Core["核心实现"]
        ITM[ITimerManager]
        TM[TimerManager]
    end

    subgraph Data["数据结构"]
        Items["_items<br/>Dictionary"]
        ToAdd["_toAdd<br/>Dictionary"]
        ToRemove["_toRemove<br/>List"]
        Pool["_pool<br/>Object Pool"]
    end

    TC -->|调用| ITM
    ITM -->|实现| TM
    TM -->|管理| Items
    TM -->|管理| ToAdd
    TM -->|管理| ToRemove
    TM -->|复用| Pool
```

## 任务添加流程

```mermaid
sequenceDiagram
    participant C as 客户端代码
    participant TC as TimerComponent
    participant TM as TimerManager
    participant Pool as 对象池

    C->>TC: Add(interval, repeat, callback, param)
    TC->>TM: Add(interval, repeat, callback, param)
    
    alt 回调为空
        TM-->>C: 输出警告日志并返回
    else 任务已存在
        TM->>TM: 更新现有任务配置
    end
    
    alt 从对象池获取
        TM->>Pool: GetFromPool()
        Pool-->>TM: TimerItem
    end
    
    TM->>TM: lock(Locker)
    TM->>TM: 设置TimerItem属性
    TM->>TM: 添加到_toAdd字典
    TM->>TM: unlock(Locker)
```

### 任务执行流程

```mermaid
flowchart TD
    Start([Update 调用]) --> CheckItems{有任务在执行?}

    CheckItems -->|否| End([返回])
    CheckItems -->|是| Iterate[遍历任务]

    Iterate --> CheckDeleted{已删除?}
    CheckDeleted -->|是| AddToRemove[添加到待移除列表]
    CheckDeleted -->|否| UpdateElapsed[更新已用时间]

    AddToRemove --> NextTask
    UpdateElapsed --> CheckTime{时间到?}
    CheckTime -->|否| NextTask
    CheckTime -->|是| Execute[执行回调]
    Execute --> DecRepeat{重复次数>0?}
    DecRepeat -->|是| Decrement[减少计数]
    DecRepeat -->|否| SetInfinite[保持无限]
    
    Decrement --> CheckZero{计数=0?}
    CheckZero -->|是| MarkDeleted[标记删除]
    CheckZero -->|否| NextTask
    SetInfinite --> NextTask

    MarkDeleted --> AddToRemove
    NextTask --> Iterate

    subgraph Cleanup["清理阶段"]
        AddToRemove --> ProcessRemove[处理待移除]
        ProcessRemove --> RemoveFromItems[从字典移除]
        RemoveFromItems --> ReturnToPool[归还对象池]
    end

    ReturnToPool --> AddPending{有待添加?}
    AddPending -->|是| ProcessAdd[处理待添加]
    AddPending -->|否| End
    ProcessAdd --> End
```

## 使用示例

### 基本定时任务

添加一个每 1 秒执行一次，共执行 10 次的任务：

```csharp
// 通过 TimerComponent 添加定时任务
var timerComponent = gameObject.GetComponent<TimerComponent>();
timerComponent.Add(1.0f, 10, callback, null);

void callback(object param)
{
    Debug.Log("定时任务执行!");
}
```

> Source: [TimerComponent.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerComponent.cs#L37-L40)

### 无限重复任务

添加一个每 2 秒执行一次的无限循环任务（repeat = 0 表示无限）：

```csharp
// repeat 为 0 表示无限重复执行
timerComponent.Add(2.0f, 0, onInfiniteTick, "myData");

void onInfiniteTick(object param)
{
    string data = param as string;
    Debug.Log($"无限任务执行, 数据: {data}");
}
```

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L156-L188)

### 带参数的任务

通过 callbackParam 传递自定义数据：

```csharp
// 传递复杂数据对象
var myData = new { Name = "Player", Score = 100 };
timerComponent.Add(1.0f, 5, onScoreUpdate, myData);

void onScoreUpdate(object param)
{
    var data = param as dynamic;
    Debug.Log($"玩家: {data.Name}, 分数: {data.Score}");
}
```

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L24-L30)

## 内部实现详解

### TimerItem 数据结构

```csharp
class TimerItem
{
    public float Interval;      // 间隔时间
    public int Repeat;           // 剩余重复次数
    public Action<object> Callback;  // 回调函数
    public object Param;        // 回调参数
    
    public float Elapsed;       // 已用时间
    public bool Deleted;        // 删除标记
}
```

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L14-L31)

### 时间精度处理

TimerManager 在 Update 中处理时间累积：

```csharp
timerItem.Elapsed += realElapseSeconds;
if (timerItem.Elapsed < timerItem.Interval)
{
    continue;  // 时间未到，跳过
}

timerItem.Elapsed -= timerItem.Interval;
// 处理时间漂移：如果 elapsed 太小或太大则重置
if (timerItem.Elapsed < 0 || timerItem.Elapsed > 0.03f)
    timerItem.Elapsed = 0;
```

这种设计确保了：
- 任务在精确的间隔后执行
- 累积的时间误差被自动修正
- 即使帧率波动也能保持相对准确

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L63-L71)

### 对象池机制

为减少垃圾回收压力，TimerManager 实现了对象池：

```csharp
private TimerItem GetFromPool()
{
    if (_pool.Count > 0)
    {
        var item = _pool[_pool.Count - 1];
        _pool.RemoveAt(_pool.Count - 1);
        item.Deleted = false;
        item.Elapsed = 0;
        return item;
    }
    return new TimerItem();
}

private void ReturnToPool(TimerItem t)
{
    t.Callback = null;
    _pool.Add(t);
}
```

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L266-L289)

### 线程安全

所有对任务字典的操作都使用 lock 保护：

```csharp
private static readonly object Locker = new object();

public void Add(float interval, int repeat, Action<object> callback, object callbackParam = null)
{
    lock (Locker)
    {
        // ... 添加任务逻辑
    }
}
```

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L38)

## 异常处理

TimerManager 提供了静态配置来控制回调异常处理：

```csharp
// 默认情况下不捕获异常，异常会直接抛出
public static bool CatchCallbackExceptions = false;

// 开启异常捕获
TimerManager.CatchCallbackExceptions = true;
```

开启后，异常会被捕获并记录警告日志，任务会被自动删除。

> Source: [TimerManager.cs](https://github.com/GameFrameX/com.gameframex.unity.timer/blob/main/Runtime/Timer/TimerManager.cs#L42-L43)

## 相关链接

- [添加单次任务](./4-core-functions.add-once) - 详细了解 AddOnce 方法
- [添加帧更新任务](./4-core-functions.add-update) - 详细了解 AddUpdate 方法
- [检查和移除任务](./4-core-functions.exists-remove) - 了解如何检查和移除任务
- [TimerComponent API 参考](./5-api-reference.timer-component) - 完整的 API 文档
- [ITimerManager 接口](./5-api-reference.itimer-manager) - 接口定义
- [TimerManager 实现](./5-api-reference.timer-manager) - 内部实现细节
