...
이 페이지에는 com.gameframex.unity.startup 저장소의 모든 버전 변경 이력이 수록되어 있으며, 저장소 루트 디렉터리의 CHANGELOG.md에서 직접 동기화된 것으로 확장 기술이나 해석은 포함하지 않습니다.
...
| 1.4.2 | 2026-07-23 | Bug Fix | asmdef: Runtime 참조 패키지의 versionDefines 매크로 정의 보완 |
| 1.4.1 | 2026-07-22 | Bug Fix | deps: systeminfo 종속성 추가 |
| 1.4.0 | 2026-07-22 | Feature | startup: GameEntry 컴포넌트 접근 모드로 마이그레이션 |
| 1.3.1 | 2026-07-22 | Bug Fix | deps: unitask 종속성 버전 고정 |
| 1.3.0 | 2026-07-18 | Feature | startup: 테넌트 인증 요청 헤더 지원 |
| 1.2.0 | 2026-07-06 | Feature | startup: WebGL 단독 실행 시 원격 시작 요청을 건너뛰도록 SkipRemoteStartupRequests 추가 |
| 1.1.0 | 2026-07-03 | Feature | editor: 시작 설정 Inspector에 필드 tooltip 표시; startup: StartupOptions 필드 확장 및 리소스 모드 동기화, 백그라운드 인증 헤더; startup: 리소스 패키지 URL은 서버에서 전달된 경로를 우선 사용 |
| 1.0.1 | 2026-06-16 | Bug Fix | ci: ci 설정 업데이트 |
| 1.0.0 | 2026-06-16 | Feature | docs/http/startup 여러 신규 항목 추가, initial commit |

## 전체 변경 로그

다음은 [CHANGELOG.md](https://github.com/GameFrameX/com.gameframex.unity.startup/blob/main/CHANGELOG.md)의 원문이며, 위에서 아래로 버전이 새로운 것부터 오래된 것 순입니다.
...
출처: [CHANGELOG.md](https://github.com/GameFrameX/com.gameframex.unity.startup/blob/main/CHANGELOG.md)