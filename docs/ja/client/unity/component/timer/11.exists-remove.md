...
在 Timer タイマーコンポーネント中，除了添加定时任务外，还提供します检查任务是否存在以及移除任务的機能。这些機能对于动态管理定时任务、避免重复添加或在特定条件下取消任务非常有用。

## 概要

TimerComponent 提供します两个コアメソッドに使用检查和移除任务：

- **Exists** - 检查指定回调函数对应的任务是否存在
- **Remove** - 移除（取消）指定回调函数对应的任务

这两个メソッド都を通じて回调函数的引用（`Action<object>`）来标识和操作任务，因此必要确保传入的是同一个委托インスタンス。

## コア機能

### 检查任务是否存在

`Exists` メソッドに使用检查指定的回调函数是否对应一个有效的定时任务。
...
1. **待添加列表（`_toAdd`）**：检查任务是否在等待添加队列中刚作成但尚未生效
2. **活跃任务列表（`_items`）**：检查任务是否在当前活跃的任务列表中
3. **删除标记（`Deleted`）**：只有未被标记为删除的任务才返回 true

### 移除指定任务

`Remove` メソッドに使用移除（取消）指定的定时任务。该メソッド采用延迟删除机制，不会立即从字典中移除，而是在下一帧 Update 时清理。
...
public void Remove(Action<object> callback)
{
    lock (Locker)
    {
        // 如果任务还在待添加队列中，直接取消并归还到オブジェクトプール
...
1. **待添加队列**：如果任务尚未开始执行（还在 `_toAdd` 中），直接从队列中移除并归还オブジェクトプール
...
```mermaid
flowchart TD
    subgraph Existsフロー["Exists 检查フロー"]
        Start1[呼び出し Exists] --> CheckToAdd{检查 _toAdd}
    ...
    subgraph Removeフロー["Remove 移除フロー"]
        Start2[呼び出し Remove] --> CheckToAdd2{在 _toAdd 中?}
        CheckToAdd2 -->|是| RemoveFromToAdd[从 _toAdd 移除<br/>归还オブジェクトプール]
...
在实际应用中，可能必要に基づいてゲームステータス动态添加和移除任务：
...
使用 Lambda 表达式时必要特别注意，每次 Lambda 表达式都会作成新的委托インスタンス，导致无法正确匹配和移除任务：

```csharp
// ❌ 错误示例：Lambda 表达式每次作成新インスタンス
_timer.Add(1.0f, 5, (param) => Debug.Log("Tick"));
_timer.Exists((param) => Debug.Log("Tick")); // 返回 False，因为是不同的委托インスタンス
_timer.Remove((param) => Debug.Log("Tick"));  // 无法移除，因为是不同的委托インスタンス

// ✅ 正确示例：使用具名メソッド或保存委托引用
...
`Exists` 和 `Remove` メソッド都使用 `lock (Locker)` 来保证线程安全。由于 TimerManager 的 Update メソッド也在锁内执行操作，这些メソッド可能安全地在任何线程上呼び出し。
...
如果在 Remove 呼び出し后立即呼び出し Exists，可能仍返回 true，因为：
1. 任务被标记为 Deleted，但尚未在下一帧 Update 中清理
2. 解決策：在 Remove 后等待一帧，或使用其他机制确认

### Lambda 表达式无法匹配任务怎么办？

如前所述，每次 Lambda 表达式会作成新的委托インスタンス。解決策是：
1. 使用具名メソッド代替 Lambda
2. 保存委托引用并复用

### 多次呼び出し Remove 会出问题吗？

不会。Remove メソッド设计为幂等操作，多次呼び出し不会产生错误。如果任务不存在，メソッド会静默返回。
