...
数据包process器是 GameFrameX 网络库的核心组件，responsible for网络消息的serialize和反serialize操作。它们将Application Layer的消息对象convert为网络字节流（send时），并将receive到的字节流convert回消息对象（receive时）。

## 概述

在网络通信中，数据需要经过特定的格式process才能在网络中传输。数据包process器正是承担这一Responsibility的关键组件，它们定义了网络消息的结构和parse方式，使得开发者可以专注于业务逻辑而无需关心Low-level Network通信细节。

GameFrameX 网络库provides了五类数据包process器：
- **send包头process器**：responsible forserialize消息头部information
- **send包体process器**：responsible forserialize消息体内容
- **receive包头process器**：responsible for反serialize消息头部information
- **receive包体process器**：responsible for反serialize消息体内容
- **心跳包process器**：responsible forprocessHeartbeat Check相关逻辑
...
上图展示了数据包process器在网络通信中的核心位置。它们throughNetwork Helper（INetworkChannelHelper）被调用，complete消息的serialize和反serialize工作。

## 消息send流程

当Client需要Send Message时，数据包process器按照特定的sequentialProcess message对象：
...
send流程分为两个主要阶段：
1. **包头serialize**：send包头process器首先被调用，它get消息的Typeinformation（消息ID），将消息对象serialize为字节数组，根据configuration决定是否compress，然后write包头information（包长度、操作Type、Compression flag、唯一ID、消息ID）
2. **包体serialize**：send包体process器将process好的包体数据write目标流

## 消息receive流程

当从网络receive到数据时，数据包process器反向process字节流，将其convert为消息对象：
...
receive流程同样分为两个阶段：
1. **包头反serialize**：receive包头process器从字节流中parse出包长度、操作Type、Compression flag、唯一ID和消息ID等information
2. **包体反serialize**：receive包体process器根据消息ID找到对应的Message Types，Uses ProtoBuf 反serialize为消息对象

## Interface Definitions

### IPacketHandler

标记接口，所有数据包process器都需要Implements此接口：
...
send包头process器接口，responsible forserialize消息头部：
...
send包体process器接口，responsible for将消息体write目标流：
...
receive包头process器接口，responsible for反serialize消息头部：
...
receive包体process器接口，responsible for反serialize消息体：
...
心跳包process器接口，responsible for心跳消息的生成和process：
...
默认send包头process器Implements了完整的包头serialize逻辑：
...
默认receive包头process器Implements了完整的包头parse逻辑：
...
默认receive包体process器，Uses ProtoBuf 反serialize消息：
...
如果默认的process器无法满足需求，开发者可以Creates自定义的数据包process器。自定义process器需要Implements相应的接口并Inherits `IPacketHandler` 标记接口。

### Creates自定义receive包头process器
...
数据包process器的行为可以through以下方式进行configuration：

| configuration项 | Type | Default | Description |
|--------|------|--------|------|
| LimitCompressLength | uint | 512 | 超过此长度时启用Message Compression |
| HeartBeatInterval | float | 5.0 | 心跳包send间隔（秒） |
| MissHeartBeatCountByClose | int | 5 | Heartbeat Lost次数上限，超过则断开connect |
