# UI 组层级

UI 组层级（UI Group Hierarchy）是 GameFrameX UI 系统中用于管理不同 UI 界面显示层次关系的核心机制。通过深度（Depth）值的配置，系统能够精确控制各个 UI 组以及组内界面的前后遮挡关系，确保 UI 界面按照预期的层级顺序进行渲染和交互。

## 概述

在复杂的游戏或应用程序中，UI 界面通常需要分层管理。例如，背景界面应该位于最底层，而弹出的提示框或系统公告应该显示在最顶层。GameFrameX UI 系统通过引入「组」的概念，结合深度（Depth）属性，实现了对 UI 界面层级的精细控制。

UI 组层级系统包含两个层次的深度控制：

- **组级别深度（Group Depth）**：控制不同 UI 组之间的显示顺序，深度值越大的组越靠前显示
- **组内深度（Form Depth in Group）**：控制同一组内各个界面之间的显示顺序

这种双层设计既保证了不同类型 UI 的整体层级关系，又允许在同类型 UI 中灵活调整具体界面的显示顺序。

### 核心设计理念

1. **深度值分层**：使用整数深度值表示层级，支持负数深度，允许更灵活的空间分配
2. **预定义组类型**：提供常用的 UI 组类型，涵盖从背景到系统弹窗的完整层级体系
3. **动态调整**：支持在运行时动态修改组深度，实时更新所有关联界面的显示层级
4. **自动刷新**：深度变更后自动触发刷新机制，确保所有界面正确响应

## 架构设计

### 核心组件关系

```mermaid
graph TD
    subgraph IUIGroup["IUIGroup 接口层"]
        Interface[IUIGroup]
    end

    subgraph UIGroupImpl["UIGroup 实现层"]
        Impl[UIGroup]
        Define[UIGroupDefine]
        Info[UIFormInfo]
    end

    subgraph UIGroupHelper["UI 组辅助器"]
        HelperInt[IUIGroupHelper]
        HelperBase[UIGroupHelperBase]
    end

    subgraph PredefinedGroups["预定义组常量"]
        Constants[UIGroupConstants]
        Names[UIGroupNameConstants]
    end

    subgraph IUIManager["UI 管理器"]
        Manager[BaseUIManager]
    end

    Interface --> Impl
    Impl --> Define
    Impl --> Info
    Impl --> HelperInt
    HelperInt --> HelperBase
    Manager --> Impl
    Constants --> Names
```

### 层级数据流

```mermaid
sequenceDiagram
    participant M as UI管理器
    participant G as UIGroup
    participant H as IUIGroupHelper
    participant F as IUIForm

    M->>G: AddUIGroup(name, depth, helper)
    G->>H: Handler(root, name, type, helper, depth)
    M->>G: OpenUIForm(form, groupName)
    G->>G: AddUIForm(form)
    Note over G: 新界面初始为 Covered=true, Paused=true
    G->>G: Refresh()
    Note over G: 遍历更新所有 form 的深度和状态
    G->>F: OnDepthChanged(groupDepth, formDepth)
    G->>F: OnReveal() / OnResume()

    rect rgb(240, 248, 255)
    Note over G: 深度变更流程
    end

    M->>G: 设置 Depth 属性
    G->>G: m_Depth = newDepth
    G->>H: SetDepth(depth)
    G->>G: Refresh()
```

### 深度值层级示意图

```mermaid
flowchart TB
    subgraph Top["高层级 (High Depth)"]
        H1["Hidden<br/>Depth: 40"]
        H2["Background<br/>Depth: 35"]
        H3["Scene<br/>Depth: 30"]
    end

    subgraph Mid["中层级 (Middle Depth)"]
        M1["World<br/>Depth: 27"]
        M2["Battle<br/>Depth: 25"]
        M3["Hud<br/>Depth: 22"]
        M4["Map<br/>Depth: 20"]
        M5["Floor<br/>Depth: 15"]
        M6["Normal<br/>Depth: 10"]
    end

    subgraph Low["低层级 (Low Depth)"]
        L1["Fixed<br/>Depth: 0"]
        L2["Window<br/>Depth: -10"]
        L3["Tip<br/>Depth: -15"]
    end

    subgraph Bottom["最底层级 (Bottom Depth)"]
        B1["Guide<br/>Depth: -20"]
        B2["Dialogue<br/>Depth: -23"]
        B3["Loading<br/>Depth: -25"]
        B4["Notify<br/>Depth: -30"]
        B5["System<br/>Depth: -35"]
    end

    Top --> Mid
    Mid --> Low
    Low --> Bottom
```

## 预定义 UI 组

GameFrameX UI 系统提供了 18 个预定义的 UI 组类型，涵盖了游戏开发中常见的 UI 场景。这些预定义组按照深度值从高到低排列：

| 组名称 | 深度值 | 用途说明 |
|--------|--------|----------|
| Hidden | 40 | 隐藏界面，用于临时存储不显示的界面 |
| Background | 35 | 背景界面，如游戏主界面背景、场景背景等 |
| Scene | 30 | 场景界面，与游戏场景相关的 UI |
| World | 27 | 世界界面，世界空间内的 UI 元素 |
| Battle | 25 | 战斗界面，战斗相关的 HUD 和信息显示 |
| Hud | 22 | 头顶显示，角色头顶的血条、名称等信息 |
| Map | 20 | 地图界面，小地图、大地图等 |
| Floor | 15 | 底板界面，位于底层的装饰性 UI |
| Normal | 10 | 普通界面，常规的游戏内 UI |
| Fixed | 0 | 固定界面，固定在屏幕上的 UI |
| Window | -10 | 窗口界面，弹出窗口、对话框等 |
| Tip | -15 | 提示界面，轻量级提示信息 |
| Guide | -20 | 引导界面，新手引导相关 UI |
| BlackBoard | -22 | 黑板界面，教学、说明类 UI |
| Dialogue | -23 | 对话界面，对话树、NPC 对话等 |
| Loading | -25 | 加载界面，加载画面 |
| Notify | -30 | 通知界面，系统通知、公告 |
| System | -35 | 系统界面，最高优先级的系统级弹窗 |

> Source: [UIGroupConstants.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UIGroupConstants.cs#L41-L201)

### 使用预定义组

```csharp
// 使用预定义组常量创建 UI 组
var uiGroupHelper = new DefaultUIGroupHelper();
uiManager.AddUIGroup(UIGroupConstants.Normal.Name, UIGroupConstants.Normal.Depth, uiGroupHelper);

// 或者使用组名称常量
uiManager.AddUIGroup(UIGroupNameConstants.Normal, 10, uiGroupHelper);
```

> Source: [BaseUIManager.UIGroup.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/BaseUIManager.UIGroup.cs#L136-L148)

## 核心机制

### 组深度管理

每个 UI 组都维护一个整数类型的 `Depth` 属性，表示该组在整体 UI 层级中的位置。当深度值发生改变时，系统会自动完成以下操作：

1. 调用 `IUIGroupHelper.SetDepth()` 方法通知辅助器更新渲染层级
2. 调用 `Refresh()` 方法刷新组内所有界面的深度信息
3. 触发各界面 `OnDepthChanged` 回调，通知界面深度已变更

```csharp
// UIGroup.cs 中的 Depth 属性实现
public int Depth
{
    get { return m_Depth; }
    set
    {
        if (m_Depth == value)
        {
            return;
        }

        m_Depth = value;
        m_UIGroupHelper.SetDepth(m_Depth);  // 通知辅助器
        Refresh();  // 刷新组内界面
    }
}
```

> Source: [UIGroup.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UI/UIGroup.cs#L106-L120)

### 组内界面深度

在同一 UI 组内，新打开的界面默认添加到列表头部（`AddFirst`），这意味着最后打开的界面会显示在最前面。每个界面在组内的深度由其在链表中的位置决定，链表头部的深度值最大，依次递减。

```csharp
// 添加新界面到组
public void AddUIForm(IUIForm uiForm)
{
    m_UIFormInfos.AddFirst(UIFormInfo.Create(uiForm));
}
```

> Source: [UIGroup.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UI/UIGroup.cs#L439-L442)

### 刷新机制

`Refresh()` 方法是 UI 组层级管理的核心，它负责：

1. **更新界面深度**：遍历组内所有界面，计算并更新每个界面的 `DepthInUIGroup`
2. **处理覆盖状态**：根据界面的显示顺序，设置 `Covered` 状态并触发相应回调
3. **处理暂停状态**：根据组的 `Pause` 属性和界面的 `PauseCoveredUIForm` 属性，管理界面的暂停状态

```csharp
public void Refresh()
{
    LinkedListNode<UIFormInfo> current = m_UIFormInfos.First;
    bool pause = m_Pause;
    bool cover = false;
    int depth = UIFormCount;

    while (current != null)
    {
        // 更新深度
        info.UIForm.OnDepthChanged(Depth, depth--);

        // 处理覆盖和暂停逻辑
        // ...
    }
}
```

> Source: [UIGroup.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UI/UIGroup.cs#L520-L577)

### 界面状态变化

UI 组内的界面有以下几种状态：

| 状态 | 说明 | 触发回调 |
|------|------|----------|
| Active | 活动状态，完全可见且可交互 | - |
| Covered | 被覆盖状态，被其他界面遮挡 | `OnCover()` / `OnReveal()` |
| Paused | 暂停状态，不再接收输入 | `OnPause()` / `OnResume()` |

当新界面打开时，原来的顶层界面会被遮挡（Covered）并可能暂停（Paused，取决于 `PauseCoveredUIForm` 属性）。

## 使用示例

### 创建自定义深度的 UI 组

```csharp
// 创建自定义深度的 UI 组
public class GameUIInitializer
{
    private IUIManager uiManager;

    public void Initialize()
    {
        // 创建高优先级组（显示在最前面）
        var dialogHelper = new DialogUIGroupHelper();
        uiManager.AddUIGroup("Dialog", 100, dialogHelper);

        // 创建普通组
        var normalHelper = new NormalUIGroupHelper();
        uiManager.AddUIGroup("Normal", 50, normalHelper);

        // 创建背景组
        var bgHelper = new BackgroundUIGroupHelper();
        uiManager.AddUIGroup("Background", 10, bgHelper);
    }
}
```

> Source: [BaseUIManager.UIGroup.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/BaseUIManager.UIGroup.cs#L136-L148)

### 动态调整组深度

```csharp
// 运行时调整 UI 组深度
public class UIGroupDepthChanger
{
    private IUIManager uiManager;

    public void PromoteGroup(string groupName)
    {
        var group = uiManager.GetUIGroup(groupName);
        if (group != null)
        {
            // 将组深度提高，使其显示在更前面
            group.Depth += 10;
        }
    }

    public void DemoteGroup(string groupName)
    {
        var group = uiManager.GetUIGroup(groupName);
        if (group != null)
        {
            // 将组深度降低，使其显示在更后面
            group.Depth -= 10;
        }
    }
}
```

### 实现自定义 UI 组辅助器

```csharp
// 自定义 UI 组辅助器
public class CustomUIGroupHelper : UIGroupHelperBase
{
    private Canvas m_Canvas;
    private int m_Depth;

    public override int Depth
    {
        get => m_Depth;
        protected set => m_Depth = value;
    }

    public override void SetDepth(int depth)
    {
        m_Depth = depth;
        if (m_Canvas != null)
        {
            // 根据组深度设置 Canvas 渲染顺序
            m_Canvas.sortingOrder = depth;
        }
    }

    public override IUIGroupHelper Handler(Transform root, string groupName, 
        string uiGroupHelperTypeName, IUIGroupHelper customUIGroupHelper, int depth = 0)
    {
        base.Handler(root, groupName, uiGroupHelperTypeName, customUIGroupHelper, depth);
        
        m_Canvas = root.GetComponent<Canvas>();
        if (m_Canvas == null)
        {
            m_Canvas = root.gameObject.AddComponent<Canvas>();
        }
        m_Canvas.renderMode = RenderMode.ScreenSpaceCamera;
        m_Canvas.sortingOrder = depth;
        
        return this;
    }
}
```

> Source: [UIGroupHelperBase.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UIGroupHelperBase.cs#L41-L77)

## 最佳实践

### 1. 合理规划深度区间

建议按照预定义组的深度分布来规划自定义组的深度：

- **高优先级区域（30+）**：用于场景背景、全屏 UI
- **中优先级区域（10-25）**：用于游戏 HUD、信息面板
- **普通区域（-5 to 10）**：用于普通弹出窗口
- **低优先级区域（-30 to -10）**：用于提示、引导类 UI

### 2. 避免深度冲突

虽然系统允许任意整数深度值，但建议使用一定间隔的深度值（如间隔 10），为未来的功能扩展预留空间。

### 3. 利用 PauseCoveredUIForm 属性

对于需要保持更新的 UI（如倒计时、实时数据），应该设置 `PauseCoveredUIForm = false`，确保即使被遮挡也能继续更新：

```csharp
// 在打开界面时设置
uiManager.OpenUIForm<MyForm>("MyForm", "Dialog", userData: null, pauseCoveredUIForm: false);
```

### 4. 正确使用预定义组

尽量使用 `UIGroupConstants` 中定义的预定义组，这样可以与其他模块保持一致的层级约定，减少自定义组的数量。

## API 参考

### IUIGroup 接口

```csharp
public interface IUIGroup
{
    // 获取界面组名称
    string Name { get; }

    // 获取或设置界面组深度
    int Depth { get; set; }

    // 获取或设置界面组是否暂停
    bool Pause { get; set; }

    // 获取界面组中界面数量
    int UIFormCount { get; }

    // 获取当前界面（组内最前面的界面）
    IUIForm CurrentUIForm { get; }

    // 获取界面组辅助器
    IUIGroupHelper Helper { get; }

    // 检查界面是否存在
    bool HasUIForm(int serialId);
    bool HasUIForm(string uiFormAssetName);

    // 获取界面
    IUIForm GetUIForm(int serialId);
    IUIForm GetUIForm(string uiFormAssetName);
    IUIForm[] GetUIForms(string uiFormAssetName);
    IUIForm[] GetAllUIForms();

    // 刷新界面组
    void Refresh();
}
```

> Source: [IUIGroup.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UI/IUIGroup.cs#L42-L197)

### IUIGroupHelper 接口

```csharp
public interface IUIGroupHelper
{
    // 获取或设置界面组深度
    int Depth { get; }

    // 设置界面组深度
    void SetDepth(int depth);

    // 创建界面组
    IUIGroupHelper Handler(Transform root, string groupName, 
        string uiGroupHelperTypeName, IUIGroupHelper customUIGroupHelper, int depth = 0);
}
```

> Source: [IUIGroupHelper.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UI/IUIGroupHelper.cs#L40-L72)

### UIGroupDefine 类

```csharp
public sealed class UIGroupDefine
{
    // 界面组深度
    public readonly int Depth;

    // 界面组名称
    public readonly string Name;

    public UIGroupDefine(int depth, string name);
}
```

> Source: [UIGroupDefine.cs](https://github.com/GameFrameX/com.gameframex.unity.ui/blob/main/Runtime/UIGroupDefine.cs#L39-L75)

## 相关链接

- [UI 组件](./4-core-concepts.ui-component) - UI 组件基础概念
- [UI 窗体](./4-core-concepts.ui-form) - UI 窗体详细说明
- [UI 组系统](./4-core-concepts.ui-group) - UI 组系统核心概念
- [IUIManager API](./5-api-reference.iuimanager) - UI 管理器接口文档
- [IUIGroup API](./5-api-reference.iuigroup) - UI 组接口文档
- [事件系统](./6-event-system) - UI 事件系统
