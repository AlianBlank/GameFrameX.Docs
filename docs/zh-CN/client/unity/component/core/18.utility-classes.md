...
GameFrameX 框架提供的核心静态工具函数库，涵盖加密解密、哈希计算、JSON序列化、压缩解压、字符串处理、随机数生成、数据校验、网络操作、ID生成等常用功能。

## 概述

Utility 工具类是 GameFrameX 框架的基础设施层，为上层业务逻辑提供底层技术支撑。这些工具类采用**静态类设计**，无需实例化即可直接调用，极大地简化了日常开发工作。

### 设计理念

1. **无状态工具方法**：所有方法都是无副作用的静态方法，可安全并行调用
2. **可扩展架构**：关键功能采用 Helper 插拔机制，支持用户自定义实现
3. **Partial Class 组织**：使用 C# partial class 特性，按功能模块分散代码到多个文件，提高可维护性
...
支持多种哈希算法，用于数据完整性校验和密码存储：

- **MD5**：128位哈希值，适用于文件完整性校验
- **SHA1**：160位哈希值
- **SHA256**：256位哈希值，安全系数更高
- **HMAC-SHA256**：带密钥的哈希，用于消息认证
...
提供 JSON 序列化和反序列化功能，支持可插拔的 Helper 实现：
...
提供 CRC32/CRC64 校验和计算，用于数据完整性验证：
...
- [辅助类](./5-runtime.helper-classes) - Unity 特定场景的辅助组件
- [扩展方法库](./5-runtime.extension-methods) - 链式调用扩展方法
- [事件系统](./5-runtime.event-system) - 事件驱动架构
