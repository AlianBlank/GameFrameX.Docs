...
本文档介绍如何在 GameFrameX Timer コンポーネント中添加各クラス定时任务。

## 概要

Timer コンポーネント提供します三种添加定时任务的方法：

1. **Add メソッド** - 添加可重复执行的定时任务
2. **AddOnce メソッド** - 添加仅执行一次的任务
3. **AddUpdate メソッド** - 添加每帧执行的任务

所有任务都を通じて TimerComponent 暴露给 Unity コンポーネント使用，内部由 TimerManager 実装具体的定时逻辑。
...
| callback | Action\<object\> | 回调函数 |
| callbackParam | object | 传递给回调的参数（可选） |
...
- `_toAdd` - 等待添加到执行队列的任务
- `_toRemove` - 等待移除的任务
- `_pool` - オブジェクトプール，に使用复用 TimerItem 对象
...
```mermaid
flowchart TD
    Start([Update 呼び出し]) --> CheckItems{有任务在执行?}
    ...
    subgraph Cleanup["清理阶段"]
        ...
        RemoveFromItems --> ReturnToPool[归还オブジェクトプール]
    end
...
添加一个每 1 秒执行一次，共执行 10 次的任务：

```csharp
// を通じて TimerComponent 添加定时任务
...
添加一个每 2 秒执行一次的无限循环任务（repeat = 0 表示无限）：
...
为减少垃圾回收压力，TimerManager 実装了オブジェクトプール：
...
TimerManager 提供します静态配置来控制回调異常处理：
...
- [添加单次任务](./4-core-functions.add-once) - 详细了解 AddOnce メソッド
- [添加帧更新任务](./4-core-functions.add-update) - 详细了解 AddUpdate メソッド
- [检查和移除任务](./4-core-functions.exists-remove) - 了解如何检查和移除任务
- [TimerComponent API 参考](./5-api-reference.timer-component) - 完整的 API 文档
- [ITimerManager インターフェース](./5-api-reference.itimer-manager) - インターフェース定义
