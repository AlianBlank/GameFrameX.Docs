...
本文档详细介绍 GameFrameX Localization 模块提供的所有公共 API インターフェース、クラス和メソッド。

## 概要

GameFrameX Localization 模块提供します一套完整的本地化解決策，包括：
- 多语言字符串管理
- 运行时语言切换
- 参数化字符串格式化
- 系统语言自动检测
...
`ILocalizationManager` 是本地化系统的コアインターフェース，定义了管理本地化字符串的所有必須操作。
...
| プロパティ | クラス型 | 説明 |
|------|------|------|
| `DefaultLanguage` | `string` | 取得或設定默认本地化语言。当加载本地化失败时使用此语言。 |
| `Language` | `string` | 取得或設定当前本地化语言。 |
| `SystemLanguage` | `string` | 取得系统语言。 |
| `DictionaryCount` | `int` | 取得当前加载的字典数量。 |
...
系统提供します 17 个泛型重载メソッド，サポート 0 到 16 个クラス型安全参数：
...
```csharp
// 示例：使用泛型 GetString 进行クラス型安全的格式化
...
`ILocalizationHelper` インターフェースに使用抽象系统语言检测逻辑，允许開発者実装自定义的语言检测方法。
...
`LocalizationHelperBase` 是本地化辅助器的抽象基底クラス，を継承 `MonoBehaviour` 并実装 `ILocalizationHelper` インターフェース。
...
`DefaultLocalizationHelper` 是默认的本地化辅助器実装，使用 .NET 的 `CultureInfo` 取得系统区域設定。
...
**设计説明：** 默认実装将系统区域信息（如 `zh-CN`）转换为下划线格式（`zh_CN`），与 `LocalizationCode` 中定义的语言代码保持一致。

---

## LocalizationComponent クラス

`LocalizationComponent` 是 Unity コンポーネント封装クラス，を継承 `GameFrameworkComponent`，提供与 Unity 生态系统无缝集成的本地化機能。
...
| `Language` | `string` | 当前本地化语言。設定此プロパティ会触发语言切换イベント。 |
| `SystemLanguage` | `string` | 取得系统语言。 |
| `DictionaryCount` | `int` | 取得字典数量。 |

### メソッド

`LocalizationComponent` 提供します与 `ILocalizationManager` 完全相同的メソッド签名，包括：
...
## LocalizationCode 静态クラス

`LocalizationCode` 提供します标准化的语言代码常量，遵循 ISO 639-1 语言代码和 ISO 3166-1 国家/地区代码标准。
